	...
	$("#INOUT_GB_CHK").val("");//대입,대출 체크용
		...
		for (var i = 0; i < rows.length; i++) {
			if($("#selectedPrintRow").val().indexOf(gridRoot1.getItemFieldAt( rows[i].rowIndex , "SLIP_NO" )) == -1){
				$("#selectedPrintRow").val(   gridRoot1.getItemFieldAt( rows[i] , "SLIP_NO" )    +","+    $("#selectedPrintRow").val()    );
				$("#P_PRINT_PUR_DT").val(   gridRoot1.getItemFieldAt( rows[i] , "DOUT_DT" )    +","+    $("#P_PRINT_PUR_DT").val()    );
				$("#P_PRINT_STR_CODE").val(   gridRoot1.getItemFieldAt( rows[i] , "DIN_STR_CODE" )    +","+    $("#P_PRINT_STR_CODE").val()    );
				$("#P_PRINT_CORP_CODE").val(   $("#CORP_CODE").val()   +","+    $("#P_PRINT_CORP_CODE").val()    );
				$("#ROUTE_GB_CHK").val(   gridRoot1.getItemFieldAt( rows[i] , "ROUTE_GB" )    +","+    $("#ROUTE_GB_CHK").val()    );
				$("#INOUT_GB_CHK").val(   gridRoot1.getItemFieldAt( rows[i] , "INOUT_GB" )    +","+    $("#INOUT_GB_CHK").val()    );
			}
		}
...
'<rMateGrid>\
	...
	<DataGrid id="dg1" sortableColumns="true" selectionMode="multipleRows" horizontalScrollPolicy="auto" lockedColumnCount="3" verticalAlign="middle" >\
			<groupedColumns>\
				<DataGridColumn dataField="DOUT_DT"  headerText="'+deliveryDate+'" textAlign="center" width="100" />\
				<DataGridColumn dataField="DIN_STR_NAME"  headerText="'+storNm+'" textAlign="center" width="120"/>\
				<DataGridColumn dataField="SLIP_NO" headerText="'+shippingNumber+'" textAlign="center" width="120"/>\
				<DataGridColumn dataField="DIN_SLIP_NO" headerText="'+orderNumber+'" textAlign="center"  width="120"/>\
				<DataGridColumn dataField="ROUTE_GB" headerText="'+routeGb+'" textAlign="center" sortable="false"  width="80"/>\
				<DataGridColumn dataField="VEN_NAME" headerText="'+venName+'" textAlign="left"  width="250"/>\
				<DataGridColumn dataField="ITM_CODE" headerText="'+itmCode+'" textAlign="center" width="120"/>\
				<DataGridColumn dataField="ITM_NAME" headerText="'+itmName+'" textAlign="left" width="250"/>\
				<DataGridColumn dataField="UNIT" headerText="'+unit+'" textAlign="center" width="80" sortable="false"/>\
				<DataGridColumn dataField="ITM_FORM" headerText="'+itmGb+'" textAlign="center" width="80" sortable="false"/>\
				<DataGridColumn dataField="DOUT_QTY" headerText="'+orderQuantity+'" textAlign="right" width="80" formatter="{numfmt}"/>\
			    <DataGridColumn dataField="DOUT_CFM_QTY" headerText="'+logisticsShipmentQuantity+'"  width="80" textAlign="right" formatter="{numfmt}"/>\
			    <DataGridColumn dataField="DOUT_CFM_DT"  headerText="'+approvedDeliveryDates+'" textAlign="center" />\
			    <DataGridColumn dataField="DIN_CFM_DT"  headerText="'+DateOfReceiptConfirmation+'" textAlign="center" />\
			    <DataGridColumn dataField="DIN_STR_CODE" headerText="'+storCode+'" textAlign="center" visible="false"  sortable="false" />\
			    <DataGridColumn dataField="VEN_CODE" headerText="'+venCode+'" textAlign="center" visible="false"  sortable="false" />\
			    <DataGridColumn dataField="ROUTE_GB" headerText="루트" textAlign="center" visible="false"  sortable="false" />\
			    <DataGridColumn dataField="INOUT_GB" headerText="대입출구분" textAlign="center" visible="false"  sortable="false" />\
			</groupedColumns>\
			<dataProvider>\
				<SpanSummaryCollection source="{$gridData}">\
					<mergingFields>\
						<SpanMergingField name="DOUT_DT"/>\
						<SpanMergingField name="DIN_STR_NAME"/>\
						<SpanMergingField name="SLIP_NO"/>\
						<SpanMergingField name="DIN_SLIP_NO"/>\
...
function ExcelDownload() {
	...
	var P_STR_NAME = $.trim($('#P_STR_NAME').val()); 
	var P_DOUT_SDT = $.trim($('#P_DOUT_SDT').val()); 
	var P_DOUT_EDT = $.trim($('#P_DOUT_EDT').val());
	var P_VEN_NAME = $.trim($('#P_VEN_NAME').val());
	var P_ITEM_NAME = $.trim($('#P_ITEM_NAME').val()); 
	var P_SLIP_NO = $.trim($('#P_SLIP_NO').val());
	var P_ROUTE_GB = $.trim($('#P_ROUTE_GB').val());
	var P_COLUMN_NAME = $.trim($('#P_COLUMN_NAME').val());
	var P_ORDERBY = $.trim($('#P_ORDERBY').val());
	 
	//엑셀호출
	$.download('/wmsOutReportExcelDown.do',"P_STR_NAME="+P_STR_NAME
															+"&P_DOUT_SDT="+P_DOUT_SDT
															+"&P_DOUT_EDT="+P_DOUT_EDT
															+"&P_VEN_NAME="+P_VEN_NAME
															+"&P_ITEM_NAME="+P_ITEM_NAME
															+"&P_SLIP_NO="+P_SLIP_NO
															+"&P_ROUTE_GB="+P_ROUTE_GB
...
function btn_print1(){
	...
	//
		...
		var P_STR_CODE		= $("#P_STR_NAME").val();
		var P_DOUT_SDT 	= $("#P_DOUT_SDT").val().replace(/-/gi,'');
		var P_DOUT_EDT 	= $("#P_DOUT_EDT").val().replace(/-/gi,'');
		var P_TEXT_DOUT_SDT 	= $("#P_DOUT_SDT").val();
		var P_TEXT_DOUT_EDT 	= $("#P_DOUT_EDT").val();
		var P_SLIP_NO		= $("#P_SLIP_NO").val();
		var P_VEN_NAME	= $("#P_VEN_NAME").val();
		var P_ITEM_NAME	= $("#P_ITEM_NAME").val();
		
		var params = "?reportMode=HTML"+"&P_CORP_CODE="+P_CORP_CODE+
							"&P_STR_CODE="+P_STR_CODE+
							"&P_DOUT_SDT="+P_DOUT_SDT+
							"&P_DOUT_EDT="+P_DOUT_EDT+
							"&P_TEXT_DOUT_SDT="+P_TEXT_DOUT_SDT+
							"&P_TEXT_DOUT_EDT="+P_TEXT_DOUT_EDT+
							"&P_SLIP_NO="+P_SLIP_NO;
							"&P_VEN_NAME="+P_VEN_NAME+
							"&P_ITEM_NAME="+P_ITEM_NAME;
							; // AIViewer 파라미터
		window.open("aireportWmsOutInfoPrint.do" + params,'AIViewer','width=1150,height=900,top=10,left=10,toolbar=no,menubar=no,lacation=no,scrollbars=no,status=no,resizable=yes');
...
{ 
	...
	//R2 선택 체크
//	var route_gb_chk =$("#selectedPrintRow").val().substring(  0,
//			$("#selectedPrintRow").val().length - 1);
//	if(route_gb_chk.indexOf("10015") != -1)
//	{
//		alert("물류센터만 출력 가능합니다.");
//		return;	
//	}else if(route_gb_chk.indexOf("10016") != -1){
//		alert("물류센터만 출력 가능합니다.");
//		return;	
//	}else if(route_gb_chk.indexOf("10018") != -1){
//		alert("물류센터만 출력 가능합니다.");
//		return;	
//	}else if(route_gb_chk.indexOf("10019") != -1){		
//		alert("물류센터만 출력 가능합니다.");
//		return;	
//	}else if(route_gb_chk.indexOf("10020") != -1){
//		alert("물류센터만 출력 가능합니다.");
//		return;	
//	}
//	
	...
	window.open("/aireportWmsInoutCenterMngPrint.do" + params,'AIViewer','width=1180,height=950,top=10,left=10,toolbar=no,menubar=no,lacation=no,scrollbars=no,status=no,resizable=yes');
