...
public void jspInit() {
	...
	//sql_supply_sale
	StringBuilder sql_supply_sale = new StringBuilder();
	sql_supply_sale.setLength(0);
	sql_supply_sale.append("SELECT A.STR_CODE ");
	sql_supply_sale.append(",      A.STR_NAME ");
	sql_supply_sale.append(",      A.VEN_CODE ");
	sql_supply_sale.append(",      A.VEN_NAME ");
	sql_supply_sale.append(",      A.SCAN_CODE ");
	sql_supply_sale.append(",      A.ITM_NAME ");
	sql_supply_sale.append(",      A.SALE_QTY ");
	sql_supply_sale.append(",      A.SPRC ");
	sql_supply_sale.append(",      A.WPRC ");
	sql_supply_sale.append(",      A.BOT_AMT ");
	sql_supply_sale.append(",      A.DC_AMT ");
	sql_supply_sale.append(",      A.MBR_DC_AMT ");
	sql_supply_sale.append(",      (NVL(A.SPRC, 0) * NVL(A.SALE_QTY, 0)) - A.DC_AMT - A.MBR_DC_AMT AS SALE_SPRC ");
	sql_supply_sale.append(",      TO_CHAR(NVL(A.WPRC, TO_CHAR('0', 'FM90.00')) * NVL(A.SALE_QTY, TO_CHAR('0', 'FM90.00')), 'FM999999999990.00') AS SALE_WPRC ");
	sql_supply_sale.append(",      NVL(A.SPRC,0)*NVL(A.SALE_QTY,0) AS SALE_SAMT ");
	sql_supply_sale.append(",      NVL(A.SALE_QTY,0) * NVL(A.SPRC,0) - NVL(A.SALE_QTY,0) * NVL(A.WPRC,0) - A.DC_AMT - A.MBR_DC_AMT AS PROPIT_AMT ");
	sql_supply_sale.append(",      DECODE(NVL(A.SALE_QTY, 0) * NVL(A.SPRC,0), 0, 0 ");
	sql_supply_sale.append(",             ROUND( ");
	sql_supply_sale.append("                     (NVL(A.SALE_QTY,0) * NVL(A.SPRC,0) - NVL(A.SALE_QTY,0) * NVL(A.WPRC,0) - NVL(A.DC_AMT,0) - NVL(A.MBR_DC_AMT,0)) ");
	sql_supply_sale.append("                     * 100 / (NVL(A.SALE_QTY,0) * NVL(A.SPRC,0) - A.DC_AMT - A.MBR_DC_AMT) ");
	sql_supply_sale.append("                    , 2 ");
	sql_supply_sale.append("                   ) ");
	sql_supply_sale.append("             ) AS PROPIT_RT ");
	sql_supply_sale.append("FROM   ( ");
	sql_supply_sale.append("        SELECT STR.STR_NAME ");
	sql_supply_sale.append("        ,      CS.VEN_CODE ");
	sql_supply_sale.append("        ,      CS.VEN_NAME ");
	sql_supply_sale.append("        ,      SSI.SCAN_CODE ");
	sql_supply_sale.append("        ,      CPC.ITM_NAME ");
	sql_supply_sale.append("        ,      SSH.STR_CODE ");
	sql_supply_sale.append("        ,      SUM(NVL(SSI.SALE_QTY, 0)) AS SALE_QTY ");
	sql_supply_sale.append("        ,      ROUND(SSI.SALE_PRC,0) AS SPRC ");
	sql_supply_sale.append("        ,      ROUND(CPS.WPRC + MAX(CPS.WVAT),0) AS WPRC ");
	sql_supply_sale.append("        ,      SUM(NVL(SSI.BOT_AMT, 0)) AS BOT_AMT ");
	sql_supply_sale.append("        ,      SUM(NVL(SSI.DC_AMT, 0)) + SUM(NVL(SSI.SPECIAL_DC_AMT, 0)) AS DC_AMT ");
	sql_supply_sale.append("        ,      SUM(NVL(SSI.MBR_DC_AMT, 0)) AS MBR_DC_AMT ");
	sql_supply_sale.append("        FROM   SL_SALE_HDR SSH ");
	sql_supply_sale.append("        ,      SL_SALE_ITEM SSI ");
	sql_supply_sale.append("        ,      CD_PRODUCT_CMN CPC ");
	sql_supply_sale.append("        ,      CD_PRODUCT_STORE CPS ");
	sql_supply_sale.append("        ,      CD_SUPPLY CS ");
	sql_supply_sale.append("        ,      CD_STORE STR ");
	sql_supply_sale.append("        WHERE  1 = 1 ");
	sql_supply_sale.append("        AND    SSH.CORP_CODE   = SSI.CORP_CODE ");
	sql_supply_sale.append("        AND    SSH.STR_CODE    = SSI.STR_CODE ");
	sql_supply_sale.append("        AND    SSH.SALE_DT     = SSI.SALE_DT ");
	sql_supply_sale.append("        AND    SSH.POS_NO      = SSI.POS_NO ");
	sql_supply_sale.append("        AND    SSH.TRXN_NO     = SSI.TRXN_NO ");
	sql_supply_sale.append("        AND    SSI.CORP_CODE   = CPC.CORP_CODE ");
	sql_supply_sale.append("        AND    SSI.ITM_CODE    = CPC.ITM_CODE ");
	sql_supply_sale.append("        AND    SSI.SCAN_CODE   = CPC.SCAN_CODE ");
	sql_supply_sale.append("        AND    SSI.CORP_CODE   = CPS.CORP_CODE ");
	sql_supply_sale.append("        AND    SSI.ITM_CODE    = CPS.ITM_CODE ");
	sql_supply_sale.append("        AND    SSI.SCAN_CODE   = CPS.SCAN_CODE ");
	sql_supply_sale.append("        AND    SSI.STR_CODE    = CPS.STR_CODE ");
	sql_supply_sale.append("        AND    CPC.CORP_CODE   = CS.CORP_CODE ");
	sql_supply_sale.append("        AND    CPC.VEN_CODE    = CS.VEN_CODE ");
	sql_supply_sale.append("        AND    SSH.STR_CODE    = STR.STR_CODE ");
	sql_supply_sale.append("        AND    SSH.CANC_FLAG  IN  ('0','2') ");
	sql_supply_sale.append("        AND    SSH.GRE_TYPE    = '00' ");
	sql_supply_sale.append("        AND    CPC.REPT_YN     = 'Y' ");
	sql_supply_sale.append("        AND    SSH.STR_CODE    = NVL('^P_STR_CODE^',SSH.STR_CODE) ");
	sql_supply_sale.append("        AND    SSH.CORP_CODE   = '^P_CORP_CODE^' ");
	sql_supply_sale.append("        AND    SSH.SALE_DT BETWEEN '^P_SALES_SD^' AND '^P_SALES_ED^' ");
	sql_supply_sale.append("        AND    CS.VEN_CODE     = NVL('^P_VEN_CODE^',CS.VEN_CODE) ");
	sql_supply_sale.append("        GROUP  BY STR.STR_NAME ");
	sql_supply_sale.append("        ,         CS.VEN_CODE ");
	sql_supply_sale.append("        ,         CS.VEN_NAME ");
	sql_supply_sale.append("        ,         SSI.SCAN_CODE ");
	sql_supply_sale.append("        ,         CPC.ITM_NAME ");
	sql_supply_sale.append("        ,         SSI.SALE_PRC ");
	sql_supply_sale.append("        ,         CPS.WPRC ");
	sql_supply_sale.append("        ,         CPS.WVAT ");
	sql_supply_sale.append("        ,         SSH.STR_CODE ");
	sql_supply_sale.append("       ) A ");
	sql_supply_sale.append("ORDER BY A.VEN_CODE ");
	sql_supply_sale.append(",        A.SCAN_CODE ");
	initializer.prepareQuery("sql_supply_sale", sql_supply_sale.toString());
	...
	sql_time.append(",	NVL(FN_GET_STR_NAME('^P_STR_CODE^','^P_CORP_CODE^'),'전체') AS STR_NAME ");
	sql_time.append(",	(SELECT TUI.USER_NM FROM TBL_USER_INFO TUI WHERE TUI.USER_ID='^P_EMP_NO^') AS USER_NM ");
...
class ReportHandler{
	...
	//-----데이터 변수 선언-----
	String AI_STRING_OBJ6 = "";
	String AI_STRING_OBJ7 = "";
	String sheet10_A1 = "";
	String sheet10_A2 = "";
	String sheet10_E2 = "";
	String sheet10_J2 = "";
	...
	private java.sql.PreparedStatement stat_supply_sale;		//ResultSet supply_sale 객체를 생성하기 위한 PreparedStatement 객체
	private ResultSet supply_sale;		//ResultSet supply_sale 객체
	...
	//-----SQL 변수 선언-----
	private StringBuilder sql_supply_sale = new StringBuilder();		//ResultSet supply_sale 객체를 생성하기 위한 쿼리문 저장
	private StringBuilder sql_time = new StringBuilder();		//ResultSet time 객체를 생성하기 위한 쿼리문 저장

	//-----쿼리 필드 변수 선언-----
	//ResultSet supply_sale 객체에서 필드 값을 받는 쿼리 필드 변수
	private String supply_sale_STR_CODE;
	private String supply_sale_STR_NAME;
	private String supply_sale_VEN_CODE;
	private String supply_sale_VEN_NAME;
	private String supply_sale_SCAN_CODE;
	private String supply_sale_ITM_NAME;
	private long supply_sale_SALE_QTY;
	private long supply_sale_SPRC;
	private long supply_sale_WPRC;
	private long supply_sale_BOT_AMT;
	private long supply_sale_DC_AMT;
	private long supply_sale_MBR_DC_AMT;
	private long supply_sale_SALE_SPRC;
	private String supply_sale_SALE_WPRC;
	private long supply_sale_SALE_SAMT;
	private long supply_sale_PROPIT_AMT;
	private double supply_sale_PROPIT_RT;

	//ResultSet time 객체에서 필드 값을 받는 쿼리 필드 변수
	private String time_TIME;
	private String time_STR_NAME;
	private String time_USER_NM;

	//파라미터 변수 선언
	String P_CORP_CODE_Param;
	String P_SALES_SD_Param;
	String P_SALES_ED_Param;
	String P_STR_CODE_Param;
	String P_VEN_NAME_Param;
	String P_TEXT_SALES_SD_Param;
	String P_VEN_CODE_Param;
	String P_TEXT_SALES_ED_Param;
	...
	void service(JspWriter out) throws Exception{
		try{    //try 구문
			...
			cleanup(supply_sale, stat_supply_sale, null, null);
	...
	private void getScriptFromv1(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			...
			//데이터 변수 할당
			sheet10_A1="협력업체매출현황";
			sheet10_A2="점포명 :"+time_STR_NAME;
			sheet10_E2=P_TEXT_SALES_SD_Param+" ~ "+P_TEXT_SALES_ED_Param;
			sheet10_J2="사원 :"+time_USER_NM;
			sheet10_K2 = time_STR_NAME;
			sheet10_L2 = time_USER_NM;
		}    //end if(bIsFirst)
	...
	private void getScriptFromfv_2(boolean bIsFirst, boolean bIsLast, boolean bIsTrue, boolean bIsPrint) throws Exception{
		...
		if(!bIsLast){   //bIsLast가 false인 경우 - 조건 밴드의 수식 계산
			//데이터 변수 할당
			sheet20_B3="소계";
			sheet20_F3=SUM(sheet20_F2, sheet20_F3);
			sheet20_G3=SUM(sheet20_G2, sheet20_G3);
			sheet20_H3=SUM(sheet20_H2, sheet20_H3);
			sheet20_I3=SUM(sheet20_I2, sheet20_I3);
			sheet20_J3=SUM(sheet20_J2, sheet20_J3);
			sheet20_K3=SUM(sheet20_K2, sheet20_K3);
			sheet20_L3=SUM(sheet20_L2, sheet20_L3);
			sheet20_M3=sheet20_L3/sheet20_K3*100;
			sheet20_M3=Double.parseDouble(String.format("%.2f",sheet20_L3/sheet20_K3*100));
		}   //end if(!bIsLast)
	}
	...
	private void getScriptFromfv_1(boolean bIsFirst, boolean bIsLast, boolean bIsTrue, boolean bIsPrint) throws Exception{
		...
		if(!bIsLast){   //bIsLast가 false인 경우 - 조건 밴드의 수식 계산
			//데이터 변수 할당
			sheet20_A4="합계";
			sheet20_F4=SUM(sheet20_F2, sheet20_F4);
			sheet20_G4=SUM(sheet20_G2, sheet20_G4);
			sheet20_H4=SUM(sheet20_H2, sheet20_H4);
			sheet20_I4=SUM(sheet20_I2, sheet20_I4);
			sheet20_J4=SUM(sheet20_J2, sheet20_J4);
			sheet20_K4=SUM(sheet20_K2, sheet20_K4);
			sheet20_L4=SUM(sheet20_L2, sheet20_L4);
			sheet20_M4=sheet20_L4*100/sheet20_K4;
			sheet20_M4=Double.parseDouble(String.format("%.2f",sheet20_L4/sheet20_K4*100));
		}   //end if(!bIsLast)
	}
	...
	private void getScriptFromfv1(boolean bIsPrint) throws Exception{
		...
		//DataSet 객체 생성
		//ResultSet supply_sale 객체의 생성
		if(supply_sale==null){
			supply_sale = stat_supply_sale.executeQuery();
		}else{
			supply_sale.beforeFirst();
		}

		//--while Loop Start--##
		while(supply_sale.next()){   //자동으로 생성되는 반복 밴드의 루프문
			//데이터 fetch
			setFieldVariableFromsupply_sale();	//RecordSet supply_sale에서 필드 값 설정

			//조건 밴드 출력 조건문
			bIsBeforePageSkip=false;
			if(!supply_sale_VEN_NAME.equals(sheet20_B2)){    //fv_2 밴드 조건문
				bIsTrue_fv_2=true;
			}else{
				bIsTrue_fv_2=false;
			}
			...
			//데이터 변수 할당
			if(!sheet20_A2.equals(supply_sale_VEN_CODE)){
				if(!bIsFirst){
					sheet20_A2_Buf = sheet20_A2;
					sheet20_A2_Flag=true;
				}
				sheet20_A2=supply_sale_VEN_CODE;
			}
			if(!sheet20_B2.equals(supply_sale_VEN_NAME) || sheet20_A2_Flag){
				if(!bIsFirst){
					sheet20_B2_Buf = sheet20_B2;
					sheet20_B2_Flag=true;
				}
				sheet20_B2=supply_sale_VEN_NAME;
			}
			sheet20_C2 = supply_sale_SCAN_CODE;
			sheet20_D2 = supply_sale_ITM_NAME;
			sheet20_E2 = supply_sale_SALE_QTY;
			sheet20_F2 = Double.parseDouble(Get0(supply_sale_SALE_WPRC));
			sheet20_G2 = supply_sale_SALE_SAMT;
			sheet20_H2 = (double)supply_sale_BOT_AMT;
			sheet20_I2 = (double)supply_sale_DC_AMT;
			sheet20_J2 = (double)supply_sale_MBR_DC_AMT;
			sheet20_K2 = (double)supply_sale_SALE_SPRC;
			sheet20_L2 = supply_sale_PROPIT_AMT;
			sheet20_M2 = supply_sale_PROPIT_RT;
	...
	private void writeScripth1( ) throws Exception{
		//스크립트문 생성
		objectWrite(DRAW_OBJECT, 676, 19, 41, 9, AI_STRING_OBJ6, "AI_STRING_OBJ6", "draw_style_1");
		objectWrite(DRAW_OBJECT, 757, 18, 45, 8, AI_STRING_OBJ7, "AI_STRING_OBJ7", "draw_style_1");
	}
	...
	private void writeScriptv1( ) throws Exception{
		...
		objectWrite(DRAW_OBJECT, nXCurPos+308, nYCurPos+32, 172, 0, "", "AI_HLINE_OBJ5", "draw_style_0");
	}
	...
	private void writeScriptfv1(  ) throws Exception{
		...
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet20_M2"), nYCurPos+mapTop.get("sheet20_M2"), mapWidth.get("sheet20_M2"), mapHeight.get("sheet20_M2"), String.valueOf(sheet20_M2), "sheet20_M2", mapStyle.get("sheet20_M2"));
	}
	...
	private void initSQLConst() throws Exception {
		stat_supply_sale = initializer.configPreparedStatement("sql_supply_sale", mapParam, conn_retail, null);

		stat_time = initializer.configPreparedStatement("sql_time", mapParam, conn_retail, null);
	}

	//-----ResultSet 객체에서 필드 값 설정 함수-----
	//ResultSet supply_sale 객체에서 필드 값 설정 함수
	private void setFieldVariableFromsupply_sale() throws Exception{
		supply_sale_STR_CODE = GetB(supply_sale.getString(1));
		supply_sale_STR_NAME = GetB(supply_sale.getString(2));
		supply_sale_VEN_CODE = GetB(supply_sale.getString(3));
		supply_sale_VEN_NAME = GetB(supply_sale.getString(4));
		supply_sale_SCAN_CODE = GetB(supply_sale.getString(5));
		supply_sale_ITM_NAME = GetB(supply_sale.getString(6));
		supply_sale_SALE_QTY = supply_sale.getLong(7);
		supply_sale_SPRC = supply_sale.getLong(8);
		supply_sale_WPRC = supply_sale.getLong(9);
		supply_sale_BOT_AMT = supply_sale.getLong(10);
		supply_sale_DC_AMT = supply_sale.getLong(11);
		supply_sale_MBR_DC_AMT = supply_sale.getLong(12);
		supply_sale_SALE_SPRC = supply_sale.getLong(13);
		supply_sale_SALE_WPRC = GetB(supply_sale.getString(14));
		supply_sale_SALE_SAMT = supply_sale.getLong(15);
		supply_sale_PROPIT_AMT = supply_sale.getLong(16);
		supply_sale_PROPIT_RT = supply_sale.getDouble(17);
	}

	//ResultSet supply_sale 객체의 필드 값 초기화 함수
	private void setFieldInitFromsupply_sale(){
		supply_sale_STR_CODE = "";
		supply_sale_STR_NAME = "";
		supply_sale_VEN_CODE = "";
		supply_sale_VEN_NAME = "";
		supply_sale_SCAN_CODE = "";
		supply_sale_ITM_NAME = "";
		supply_sale_SALE_QTY = 0L;
		supply_sale_SPRC = 0L;
		supply_sale_WPRC = 0L;
		supply_sale_BOT_AMT = 0L;
		supply_sale_DC_AMT = 0L;
		supply_sale_MBR_DC_AMT = 0L;
		supply_sale_SALE_SPRC = 0L;
		supply_sale_SALE_WPRC = "";
		supply_sale_SALE_SAMT = 0L;
		supply_sale_PROPIT_AMT = 0L;
		supply_sale_PROPIT_RT = 0;
	}

	//ResultSet time 객체에서 필드 값 설정 함수
	private void setFieldVariableFromtime() throws Exception{
		time_TIME = GetB(time.getString(1));
		time_STR_NAME = GetB(time.getString(2));
		time_USER_NM = GetB(time.getString(3));
	}
	...
	private void initConstVarientEx(){
		AI_STRING_OBJ7="Page : #PAGE/#TOTALPAGE";
	}
	...
	private void setParam() {
		...
		P_SALES_SD_Param=decoder.getParameterB("P_SALES_SD");
		//P_SALES_SD_Param=aiReplace(P_SALES_SD_Param, str, "\n");
		mapParam.put("P_SALES_SD", P_SALES_SD_Param);

		P_SALES_ED_Param=decoder.getParameterB("P_SALES_ED");
		//P_SALES_ED_Param=aiReplace(P_SALES_ED_Param, str, "\n");
		mapParam.put("P_SALES_ED", P_SALES_ED_Param);
		...
		P_TEXT_SALES_SD_Param=decoder.getParameterB("P_TEXT_SALES_SD");
		//P_TEXT_SALES_SD_Param=aiReplace(P_TEXT_SALES_SD_Param, str, "\n");
		mapParam.put("P_TEXT_SALES_SD", P_TEXT_SALES_SD_Param);
		...
		P_TEXT_SALES_ED_Param=decoder.getParameterB("P_TEXT_SALES_ED");
		//P_TEXT_SALES_ED_Param=aiReplace(P_TEXT_SALES_ED_Param, str, "\n");
		mapParam.put("P_TEXT_SALES_ED", P_TEXT_SALES_ED_Param);
	...
	private void getScriptHeader(String paper, String size, String value, String orient, String margin) throws Exception{
		...
		manager.scriptWrite("AIReport", "scriptVersion|docVersion|style|title", "32|30|supplySalesStatePrint.style|회원매출상세내역현황", false);
	}
	...
	public void setCellInfoMap() throws Exception {
		...
		setCellInfoMap_Internal("sheet10_A1", 0, 0, 788, 32, "cell_style_0");
		setCellInfoMap_Internal("sheet10_A2", 0, 32, 80, 22, "cell_style_1");
		setCellInfoMap_Internal("sheet10_E2", 320, 32, 160, 22, "cell_style_1");
		setCellInfoMap_Internal("sheet10_J2", 720, 32, 68, 22, "cell_style_1");
		setCellInfoMap_Internal("sheet20_A1", 0, 0, 41, 20, "cell_style_3");
		setCellInfoMap_Internal("sheet20_B1", 41, 0, 90, 20, "cell_style_3");
		setCellInfoMap_Internal("sheet20_C1", 131, 0, 74, 20, "cell_style_3");
		setCellInfoMap_Internal("sheet20_D1", 205, 0, 126, 20, "cell_style_3");
		setCellInfoMap_Internal("sheet20_E1", 331, 0, 29, 20, "cell_style_3");
		setCellInfoMap_Internal("sheet20_F1", 360, 0, 59, 20, "cell_style_3");
		setCellInfoMap_Internal("sheet20_G1", 419, 0, 52, 20, "cell_style_3");
		setCellInfoMap_Internal("sheet20_H1", 471, 0, 52, 20, "cell_style_3");
		setCellInfoMap_Internal("sheet20_I1", 523, 0, 52, 20, "cell_style_3");
		setCellInfoMap_Internal("sheet20_J1", 575, 0, 52, 20, "cell_style_3");
		setCellInfoMap_Internal("sheet20_K1", 627, 0, 59, 20, "cell_style_4");
		setCellInfoMap_Internal("sheet20_L1", 686, 0, 59, 20, "cell_style_3");
		setCellInfoMap_Internal("sheet20_M1", 745, 0, 43, 20, "cell_style_5");
		setCellInfoMap_Internal("sheet20_A2", 0, -1, 41, -1, "cell_style_6");
		setCellInfoMap_Internal("sheet20_B2", 41, -1, 90, -1, "cell_style_7");
		setCellInfoMap_Internal("sheet20_C2", 131, 0, 74, 15, "cell_style_6");
		setCellInfoMap_Internal("sheet20_D2", 205, 0, 126, 15, "cell_style_7");
		setCellInfoMap_Internal("sheet20_E2", 331, 0, 29, 15, "cell_style_8");
		setCellInfoMap_Internal("sheet20_F2", 360, 0, 59, 15, "cell_style_9");
		setCellInfoMap_Internal("sheet20_G2", 419, 0, 52, 15, "cell_style_8");
		setCellInfoMap_Internal("sheet20_H2", 471, 0, 52, 15, "cell_style_8");
		setCellInfoMap_Internal("sheet20_I2", 523, 0, 52, 15, "cell_style_8");
		setCellInfoMap_Internal("sheet20_J2", 575, 0, 52, 15, "cell_style_8");
		setCellInfoMap_Internal("sheet20_K2", 627, 0, 59, 15, "cell_style_10");
		setCellInfoMap_Internal("sheet20_L2", 686, 0, 59, 15, "cell_style_8");
		setCellInfoMap_Internal("sheet20_M2", 745, 0, 43, 15, "cell_style_11");
		setCellInfoMap_Internal("sheet20_A3", 0, 0, 41, 15, "cell_style_12");
		setCellInfoMap_Internal("sheet20_B3", 41, 0, 319, 15, "cell_style_13");
		setCellInfoMap_Internal("sheet20_F3", 360, 0, 59, 15, "cell_style_14");
		setCellInfoMap_Internal("sheet20_G3", 419, 0, 52, 15, "cell_style_15");
		setCellInfoMap_Internal("sheet20_H3", 471, 0, 52, 15, "cell_style_15");
		setCellInfoMap_Internal("sheet20_I3", 523, 0, 52, 15, "cell_style_16");
		setCellInfoMap_Internal("sheet20_J3", 575, 0, 52, 15, "cell_style_15");
		setCellInfoMap_Internal("sheet20_K3", 627, 0, 59, 15, "cell_style_17");
		setCellInfoMap_Internal("sheet20_L3", 686, 0, 59, 15, "cell_style_15");
		setCellInfoMap_Internal("sheet20_M3", 745, 0, 43, 15, "cell_style_18");
		setCellInfoMap_Internal("sheet20_A4", 0, 0, 360, 17, "cell_style_19");
		setCellInfoMap_Internal("sheet20_F4", 360, 0, 59, 17, "cell_style_20");
		setCellInfoMap_Internal("sheet20_G4", 419, 0, 52, 17, "cell_style_21");
		setCellInfoMap_Internal("sheet20_H4", 471, 0, 52, 17, "cell_style_21");
		setCellInfoMap_Internal("sheet20_I4", 523, 0, 52, 17, "cell_style_21");
		setCellInfoMap_Internal("sheet20_J4", 575, 0, 52, 17, "cell_style_21");
		setCellInfoMap_Internal("sheet20_K4", 627, 0, 59, 17, "cell_style_22");
		setCellInfoMap_Internal("sheet20_L4", 686, 0, 59, 17, "cell_style_21");
		setCellInfoMap_Internal("sheet20_M4", 745, 0, 43, 17, "cell_style_23");
	}
}
%>
