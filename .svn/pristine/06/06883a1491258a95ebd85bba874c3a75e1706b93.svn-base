...
public void jspInit() {
	...
	sql_ORD_HDR.append(",      CC.CUST_NAME ");
	sql_ORD_HDR.append(",      NVL(CC.OWN_NAME, CC.CUST_NAME) AS OWN_NAME ");
	sql_ORD_HDR.append(",      CASE ");
	...
	sql_ORD_HDR.append("AND    CC.CORP_CODE     = CCC.CORP_CODE(+) ");
	sql_ORD_HDR.append("AND    CC.CUST_NO       = CCC.CUST_NO(+) ");
	sql_ORD_HDR.append("AND    CCC.USE_YN(+)    = 'Y' ");
	...
	sql_ORD_DTL.append("SELECT ROWNUM AS NO ");
	sql_ORD_DTL.append(",      COD.LINK_CODE ");
	sql_ORD_DTL.append(",      COD.ITM_NAME ");
	sql_ORD_DTL.append(",      COD.UNIT ");
	sql_ORD_DTL.append(",      COD.IPSU_QTY ");
	sql_ORD_DTL.append(",      COD.ORD_QTY ");
	sql_ORD_DTL.append(",      COD.WPRC ");
	sql_ORD_DTL.append(",      COD.SPRC ");
	sql_ORD_DTL.append(",      COD.REMARK ");
	sql_ORD_DTL.append(",      COD.ITM_CODE ");
	sql_ORD_DTL.append(",      FN_GET_COMMON_CODE_NM('ITM_FORM',CPC.ITM_FORM,COD.CORP_CODE) AS ITM_FORM_NM ");
	sql_ORD_DTL.append(",      FN_GET_COMMON_CODE_NM('ORD_FLAG',COD.ORD_FLAG,COD.CORP_CODE) AS ORD_FLAG_NM ");
	sql_ORD_DTL.append("FROM   CS_ORDER_DTL COD ");
	sql_ORD_DTL.append(",      CD_PRODUCT_CMN CPC ");
	sql_ORD_DTL.append("WHERE  COD.ITM_CODE     = CPC.ITM_CODE ");
	sql_ORD_DTL.append("AND    COD.LINK_CODE    = CPC.SCAN_CODE ");
	sql_ORD_DTL.append("AND    COD.CORP_CODE    = CPC.CORP_CODE ");
	sql_ORD_DTL.append("AND    COD.CORP_CODE    = '^P_CORP_CODE^' ");
	sql_ORD_DTL.append("AND    COD.ORD_DT       = REPLACE('^P_ORD_DT^','-','') ");
	sql_ORD_DTL.append("AND    COD.SLIP_NO      = '^P_SLIP_NO^' ");
...
class ReportHandler{
	...
	private Connection conn_umac; 	 //umac Connection 객체
	...
	private String ORD_HDR_MBR_CARD_NO;
	...
	void service(JspWriter out) throws Exception{
		try{    //try 구문
			...
			cleanup(null, null, null, conn_umac);
		}    //end try
	}
	...
	private void getScriptFromv2(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			...
			sheet20_H3="상호";
			sheet20_I3 = ORD_HDR_BUSI_NAME;
			sheet20_J3="카드번호";
			sheet20_K3 = ORD_HDR_MBR_CARD_NO;
			sheet20_M3="배달시간";
	...
	private void writeScriptv1( ) throws Exception{
		...
		objectWrite(DRAW_OBJECT, nXCurPos+279, nYCurPos+33, 234, 39, AI_BARCODE_OBJ6, "AI_BARCODE_OBJ6", "draw_style_0");
	}
	...
	private void createConnAndStateObject() throws Exception{
		//umac Connection 객체 생성
		Class.forName("oracle.jdbc.OracleDriver");
		conn_umac = DriverManager.getConnection("jdbc:oracle:thin:@210.219.229.37:1521:umac", "UMACSM", "dbaor!@#");
	}
	...
	private void initSQLConst() throws Exception {
		stat_ORD_HDR = initializer.configPreparedStatement("sql_ORD_HDR", mapParam, conn_umac, null);

		stat_ORD_DTL = initializer.configPreparedStatement("sql_ORD_DTL", mapParam, conn_umac, null);
	}
	...
	private void setFieldVariableFromORD_HDR() throws Exception{
		...
		ORD_HDR_MBR_CARD_NO = GetB(ORD_HDR.getString(31));
	}
	...
	private void setFieldVariableFromORD_DTL() throws Exception{
		...
		ORD_DTL_LINK_CODE = GetB(ORD_DTL.getString(2));
		ORD_DTL_ITM_NAME = GetB(ORD_DTL.getString(3));
		ORD_DTL_UNIT = GetB(ORD_DTL.getString(4));
		ORD_DTL_IPSU_QTY = ORD_DTL.getLong(5);
		ORD_DTL_ORD_QTY = ORD_DTL.getLong(6);
		ORD_DTL_WPRC = ORD_DTL.getDouble(7);
		ORD_DTL_SPRC = ORD_DTL.getLong(8);
		ORD_DTL_REMARK = GetB(ORD_DTL.getString(9));
		ORD_DTL_ITM_CODE = GetB(ORD_DTL.getString(10));
		ORD_DTL_ITM_FORM_NM = GetB(ORD_DTL.getString(11));
		ORD_DTL_ORD_FLAG_NM = GetB(ORD_DTL.getString(12));
	}
	...
	public void setCellInfoMap() throws Exception {
		...
		setCellInfoMap_Internal("sheet20_A2", 0, 15, 21, 80, "cell_style_3");
		setCellInfoMap_Internal("sheet20_B2", 21, 15, 51, 20, "cell_style_4");
		setCellInfoMap_Internal("sheet20_C2", 72, 15, 90, 20, "cell_style_5");
		setCellInfoMap_Internal("sheet20_D2", 162, 15, 51, 20, "cell_style_4");
		setCellInfoMap_Internal("sheet20_E2", 213, 15, 90, 20, "cell_style_6");
		setCellInfoMap_Internal("sheet20_G2", 308, 15, 21, 80, "cell_style_3");
		setCellInfoMap_Internal("sheet20_H2", 329, 15, 51, 20, "cell_style_4");
		setCellInfoMap_Internal("sheet20_I2", 380, 15, 90, 20, "cell_style_5");
		setCellInfoMap_Internal("sheet20_J2", 470, 15, 51, 20, "cell_style_4");
		setCellInfoMap_Internal("sheet20_K2", 521, 15, 90, 20, "cell_style_6");
		setCellInfoMap_Internal("sheet20_M2", 616, 15, 51, 20, "cell_style_7");
		setCellInfoMap_Internal("sheet20_N2", 667, 15, 121, 20, "cell_style_8");
		setCellInfoMap_Internal("sheet20_B3", 21, 35, 51, 20, "cell_style_9");
		setCellInfoMap_Internal("sheet20_C3", 72, 35, 90, 20, "cell_style_10");
		setCellInfoMap_Internal("sheet20_D3", 162, 35, 51, 20, "cell_style_9");
		setCellInfoMap_Internal("sheet20_E3", 213, 35, 90, 20, "cell_style_11");
		setCellInfoMap_Internal("sheet20_H3", 329, 35, 51, 20, "cell_style_9");
		setCellInfoMap_Internal("sheet20_I3", 380, 35, 90, 20, "cell_style_10");
		setCellInfoMap_Internal("sheet20_J3", 470, 35, 51, 20, "cell_style_9");
		setCellInfoMap_Internal("sheet20_K3", 521, 35, 90, 20, "cell_style_11");
		setCellInfoMap_Internal("sheet20_M3", 616, 35, 51, 20, "cell_style_12");
		setCellInfoMap_Internal("sheet20_N3", 667, 35, 121, 20, "cell_style_13");
		setCellInfoMap_Internal("sheet20_B4", 21, 55, 51, 20, "cell_style_9");
		setCellInfoMap_Internal("sheet20_C4", 72, 55, 231, 20, "cell_style_11");
		setCellInfoMap_Internal("sheet20_H4", 329, 55, 51, 20, "cell_style_9");
		setCellInfoMap_Internal("sheet20_I4", 380, 55, 231, 20, "cell_style_11");
		setCellInfoMap_Internal("sheet20_M4", 616, 55, 51, 20, "cell_style_12");
		setCellInfoMap_Internal("sheet20_N4", 667, 55, 121, 20, "cell_style_13");
		setCellInfoMap_Internal("sheet20_B5", 21, 75, 51, 20, "cell_style_14");
		setCellInfoMap_Internal("sheet20_C5", 72, 75, 90, 20, "cell_style_15");
		setCellInfoMap_Internal("sheet20_D5", 162, 75, 51, 20, "cell_style_14");
		setCellInfoMap_Internal("sheet20_E5", 213, 75, 90, 20, "cell_style_16");
		setCellInfoMap_Internal("sheet20_H5", 329, 75, 51, 20, "cell_style_14");
		setCellInfoMap_Internal("sheet20_I5", 380, 75, 90, 20, "cell_style_15");
		setCellInfoMap_Internal("sheet20_J5", 470, 75, 51, 20, "cell_style_14");
		setCellInfoMap_Internal("sheet20_K5", 521, 75, 90, 20, "cell_style_16");
		setCellInfoMap_Internal("sheet20_M5", 616, 75, 51, 20, "cell_style_17");
		setCellInfoMap_Internal("sheet20_N5", 667, 75, 121, 20, "cell_style_18");
