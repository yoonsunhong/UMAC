...
function secondLabelFunc2(item, value, column) {
	if ( item.DOUT_CFM_DT != "-"    &&  item.DIN_CFM_DT != "-" )
		return false;
	else
		return true;
}
...
'<rMateGrid>\
	...
	<DataGrid id="dg1" sortableColumns="true" editable="false" selectionMode="singleRow">\
		<groupedColumns>\
			...
			<DataGridColumn id="DOUT_TOTAL"		dataField="DOUT_TOTAL"    headerText="대출합계"  				textAlign="right"  editable="false" formatter="{numfmt}" width="200" />\
...
var layoutStr2 =
	...
	<DataGrid id="dg2" editable="true" horizontalScrollPolicy="auto" showDeletedRows="true" itemEditBeginningJsFunction="itemFunc" selectionMode="singleRow">\
		<columns>\
...
//	{   
//		if(gridRoot2.getItemFieldAt(i, "DOUT_QTY") == "" || gridRoot2.getItemFieldAt(i, "DOUT_QTY") == "0" || gridRoot2.getItemFieldAt(i, "DOUT_QTY") == undefined ||
...
//	 alert (   gridRoot2.getItemFieldAt( selectedIndex , "SLIP_NO" )   );
	...
	jQuery.ajax({ 
		    url:"/productCheckOutCancel.do",          
		    type:"POST",
			datatype:"xml",
			async:true,
			data: {    "SLIP_NO"     : gridRoot1.getItemFieldAt( selectedIndex , "SLIP_NO")
		    }, 
			success:function(data){   
...
function btnDel()
{
	...
	var chkArr = selectorColumn.getSelectedIndices() ; 
	var ERR_CODE = 0;  
	if( chkArr.length == 0)
	{   alert("점간 대출입 삭제 처리 건이 체크되지 않았습니다.");
		return;
	}
	
	if (confirm("총 "+chkArr.length+" 건의 점간 대출입 데이터를  삭제 처리  하시겠습니까?") == false){  
		return;
	}
	...
	{  
		...
		//  점간 대출입 삭제처리   
		jQuery.ajax({ 
		    url:"/productCheckOutDelete.do",           
		    type:"POST",
			datatype:"xml",
			async:true,
			data: {    
		             "SLIP_NO"     : gridRoot1.getItemFieldAt( chkArr[i] , "SLIP_NO")
		    }, 
			success:function(data){   
				if(  data[0].RETURN_CODE  != "0000")
	...
	if(ERR_CODE == 0)
	{
		alert('점간 대출입 전표 삭제 처리에 성공 하였습니다.');
	} else {
		alert('점간 대출입 전표 삭제 처리에 실패 하였습니다.'); 
	}
...
function btnSearch(){
	...
	jQuery.ajax({ 
	    url:"/selectProductCheckOutDel.do",         
	    type:"POST",
		datatype:"json",
		data: loadData, 
		success:function(data){   
...
function btnSearchDetail(){
	...
	jQuery.ajax({ 
	    url:"/selectProductCheckOutDtlDel.do",                
	    type:"POST",
		datatype:"json",
		data: {
