...
function itemFunc(rowIndex, columnIndex, item, dataField){
	...
	if(gridRoot1.getItemFieldAt(rowIndex, "ZONE_CODE") ==   "ZZ")
	{
		return false;
	} else { 
		return true;
...
'<rMateGrid>\
	...
	<NumberFormatter id="numfmt" useThousandsSeparator="true"/>\
		<DataGrid id="dg1" sortableColumns="true"    editable="true" selectionMode="singleRow" itemEditBeginningJsFunction="itemFunc" verticalAlign="middle" showDataTips="true">\
			<columns>\
				<DataGridRowStateColumn id="rowState" dataField=" "  textAlign="center"  visible="false"  />\
				<DataGridColumn dataField="ZONE_CODE" 		headerText="ZONE_CODE" 		editable="false" 	visible="false"  />\
				<DataGridColumn dataField="ZONE_NAME" 		headerText="ZONE" 			editable="false"	textAlign="center" width="70" />\
				<DataGridColumn dataField="RACK_CODE" 		headerText="RACK_CODE"  	editable="false"	visible="false" sortable="true"/>\
				<DataGridColumn dataField="RACK_NAME" 		headerText="RACK" 			editable="false"	textAlign="center"  width="70"  />\
				<DataGridColumn dataField="LINE_CODE" 		headerText="LINE_CODE" 		editable="false"	visible="false"  />\
				<DataGridColumn dataField="LINE_NAME" 		headerText="LINE" 			editable="false"	textAlign="center" width="70"  />\
				<DataGridColumn dataField="VEN_NAME" 		headerText="협력업체" 			editable="false"	width="190" textAlign="left"/>\
				<DataGridColumn dataField="ITM_CODE" 		headerText="'+itmCode+'" 	editable="false" 	textAlign="center"   width="120"/>\
				<DataGridColumn dataField="SCAN_CODE" 		headerText="'+scanCode+'" 	editable="false" 	textAlign="center" width="120"  />\
				<DataGridColumn dataField="ITM_NAME" 		headerText="'+itmName+'" 	editable="false" 	textAlign="left" />\
				<DataGridColumn dataField="UNIT" 			headerText="단위" 			editable="false" 	textAlign="center"   width="70" />\
			    <DataGridColumn dataField="CUR_INV_QTY" 	headerText="현재고" 			editable="false" 	textAlign="right"   formatter="{numfmt}" width="80" />\
			    <DataGridColumn dataField="VALID_END_DT" 	headerText="유효일자" 		textAlign="left"  	width="120"   formatter="{datefmt}"  editorProperties=\'{"editable":true}\'   itemEditor="DateEditor"   showEditableIcon="always" editableIcon="calendar" itemRenderer="EditableIconItem"	 />\
			    <DataGridColumn dataField="SEQ" 			headerText="SEQ" 		visible="false" />\
			    <DataGridColumn dataField="PUR_DT" 			headerText="PUR_DT" 	visible="false"/>\
...
//	loadData.P_INV_DT = loadData.P_INV_DT.replace(/-/gi, '');	
	...
	jQuery.ajax({ 
	    url:"/getWmsStockLocationItemList.do",         
	    type:"POST",
		datatype:"json",
...
{ 
	...
	if (confirm("유효일자를 수정 하시겠습니까?") == false){  
		return;
	}
	...
	{ 
		...
		for (var i = 0; i < changedData.length; i++)
			...
			if(  changedData[i].job  == "U")
			{
				...
				var STR_CODE 		= 	gridRoot1.getItemFieldAt( changedData[i].idx , "STR_CODE")  ;
				var PUR_DT			=	gridRoot1.getItemFieldAt( changedData[i].idx , "PUR_DT") ;
				var SEQ 			=	gridRoot1.getItemFieldAt( changedData[i].idx , "SEQ")  ;
				var VALID_END_DT 	= 	gridRoot1.getItemFieldAt( changedData[i].idx , "VALID_END_DT")   ; 
						 
				jQuery.ajax({ 
				    url:"/updateValidEndDt.do",        
				    type:"POST",
					datatype:"xml",
					async:true,
					data: {
					...
					,	"PUR_DT"        : PUR_DT  
					,   "SEQ"           : SEQ
					,   "VALID_END_DT"  : VALID_END_DT.replace(/-/gi, '')  
				    }, 
					success:function(data){   
...
//	var VEN_NAME = $.trim( dataRow1.VEN_NAME );
	dataGrid1.exportFileName = "상품재고조회.xlsx" ;  
	gridRoot1.excelExportSave("", false);
...
//	showLoadingBar1();
//	//엑셀호출
//	$.download('/wmsStockLocationItemExcelDown.do',"P_STR_CODE="+P_STR_CODE 
...
//	//로딩바 숨기기를 위한 url호출
//	jQuery.ajax({ 
//	    url:"/wmsStockLocationItemExcelDown.do",         
//	    type:"POST",
//		datatype:"json",
//		data: loadData,
//		success:function(data){  
