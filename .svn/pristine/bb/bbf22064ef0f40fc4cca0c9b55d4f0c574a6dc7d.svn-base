...
var layoutStr1 =
	'<rMateGrid>\
		...
		<DataGrid id="dg1" sortableColumns="true" lockedColumnCount="1">\
			<groupedColumns>\
				<DataGridColumn dataField="STR_NAME" headerText="'+storNm+'" width="150" textAlign="center"/>\
				<DataGridColumnGroup headerText="'+selngAmount+'">\
				...
				<DataGridColumnGroup headerText="'+selngProfitAm+'">\
					<DataGridColumn dataField="SALE_AMT" 	 		 headerText="'+goal+'"  		textAlign="right" formatter="{numfmt}"/>\
					<DataGridColumn dataField="SDOUT_SPRC" 	 		 headerText="'+acmSlt+'"  		textAlign="right" formatter="{numfmt}"/>\
					<DataGridColumn headerText="'+selngProfitPer+'"  	textAlign="right" labelJsFunction="perFunction3"/>\
					...
					<GroupingField name="TOTAL_NAME">\
					    <summaries>\
					        <SummaryRow summaryPlacement="group">\
								...
								<SummaryField dataField="SDOUT_SPRC" summaryOperation="SUM" label="SDOUT_SPRC" />\
					        </SummaryRow>\
					    </summaries>\
					</GroupingField>\
...
var layoutStr2 =
	...
	<DataGrid id="dg1" sortableColumns="true" lockedColumnCount="1">\
		<groupedColumns>\
			<DataGridColumn dataField="MID_NAME" headerText="'+storNm+'" width="150" textAlign="center"/>\
			<DataGridColumnGroup headerText="'+selngAmount+'">\
				<DataGridColumn dataField="GOAL_AMT"   			 headerText="'+goal+'"    	textAlign="right" formatter="{numfmt}"/>\
				<DataGridColumn dataField="SALE_SPRC" 	 		 headerText="'+acmSlt+'"  		textAlign="right" formatter="{numfmt}"/>\
				<DataGridColumn headerText="'+achivPer+'"  		 textAlign="right" labelJsFunction="perFunction1"/>\
...
function getGridData() {
	...
	jQuery.ajax({ 
	    url:"/salesAnalReportGoalList.do",
	    type:"POST",
		datatype:"json",
		//async:false,
		data: params,
...
function getGridData2(num1) {
	jQuery.ajax({ 
	    url:"/salesAnalReportGoalIList.do",
	    type:"POST",
		datatype:"json",
		//async:false,
		//data: params,
...
function perFunction2(item, value, column){	
	var num1 = (item["SDOUT_SPRC"] / item["SALE_AMT"]) * 100;
	return perFunction(num1);  	
}

function perFunction3(item, value, column){	
	var num1 = (item["SDOUT_SPRC"] / item["SALE_SPRC"]) * 100;		
	return perFunction(num1);  	
}
...
function excelExport(){    
	...
	dataGrid1.exportFileName = goalContrastAchiv+"_"+date+".xlsx";
