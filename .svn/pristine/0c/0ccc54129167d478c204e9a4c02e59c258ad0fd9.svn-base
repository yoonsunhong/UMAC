...
************************************************************************************
보고서 제목 : 보고서 (2017/4/24 11:10:41)
작성일 : 2017년4월24일 11시10분41초
수정일 : 2017년6월14일 17시27분4초
작성자 :
************************************************************************************/
%>
...
public void jspInit() {
	...
	//sql_pos_receipt
	StringBuilder sql_pos_receipt = new StringBuilder();
	sql_pos_receipt.setLength(0);
	sql_pos_receipt.append("SELECT  RECEIPT ");
	sql_pos_receipt.append(",	TO_CHAR(SYSDATE,'YYYY-MM-DD HH:MI') AS TIME ");
	sql_pos_receipt.append("FROM    SL_SALE_JNL S ");
	sql_pos_receipt.append("WHERE   S.CORP_CODE ='^P_CORP_CODE^' ");
	sql_pos_receipt.append("AND     S.SALE_DT   ='^P_SALE_DT^' ");
	sql_pos_receipt.append("AND     S.STR_CODE  ='^P_STR_CODE^' ");
	sql_pos_receipt.append("AND     S.POS_NO    ='^P_POS_NO^' ");
	sql_pos_receipt.append("AND     S.TRXN_NO   ='^P_TRXN_NO^' ");
	initializer.prepareQuery("sql_pos_receipt", sql_pos_receipt.toString());
}
...
class ReportHandler{
	...
	//-----데이터 변수 선언-----
	String sheet10_B1 = "";
	int[] nAutoHeight_test=new int[1];
	String sheet10_B2 = "";

	//-----Connection, Statement, ResultSet 객체 선언-----
	private Connection conn_retail; 	 //retail Connection 객체
	private java.sql.PreparedStatement stat_pos_receipt;		//ResultSet pos_receipt 객체를 생성하기 위한 PreparedStatement 객체
	private ResultSet pos_receipt;		//ResultSet pos_receipt 객체

	//-----SQL 변수 선언-----
	private StringBuilder sql_pos_receipt = new StringBuilder();		//ResultSet pos_receipt 객체를 생성하기 위한 쿼리문 저장

	//-----쿼리 필드 변수 선언-----
	//ResultSet pos_receipt 객체에서 필드 값을 받는 쿼리 필드 변수
	private String pos_receipt_RECEIPT;
	private String pos_receipt_TIME;
	...
	void service(JspWriter out) throws Exception{
		try{    //try 구문
			...
			//DataSet Close
			cleanup(pos_receipt, stat_pos_receipt, null, null);
	...
	private void executeSheet10( ) throws Exception{
		//쉬트 초기화 함수 호출
		getSheetPaperInfo("A4", "2100x2970", "9", "VERTICAL", "14 0 14 0");
		initSheetVariant(27, 13, 571, 832, true);

		//시작 페이지 번호 출력
		pageWrite(nPageNum);

		//밴드 함수 호출
		getScriptFromvv3(true, true);    //Void 밴드 호출
		getScriptFromtest(true);    //반복 밴드 호출
		getScriptFromtest2(true, true);    //Void 밴드 호출
	...
	private void getScriptFromvv3(boolean bIsFirst, boolean bIsPrint) throws Exception{
		//지역 변수 선언 및 초기화
	...
	//-----test RepeatBand Start-----
	//-----소속 쉬트 : SHEET=Sheet1   FRAME=Frame0
	//-----밴드 이름 : test
	//-----밴드 종류 : 반복 밴드
	//-----bIsPrint : 감추기 속성 설정 여부 true면 출력, false면 스크립트를 생성하지 않음
	private void getScriptFromtest(boolean bIsPrint) throws Exception{
		//지역 변수 선언 및 초기화
		...
		//폰트 사이즈 : 9
		//각 문자별 폭을 할당할 배열 선언
		int[] charWidth = {450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,450,900};

		//데이터 변수 초기화
		sheet10_B2="";

		//DataSet 객체 생성
		//ResultSet pos_receipt 객체의 생성
		if(pos_receipt==null){
			pos_receipt = stat_pos_receipt.executeQuery();
		}else{
			pos_receipt.beforeFirst();
		}

		//--while Loop Start--##
		while(pos_receipt.next()){   //자동으로 생성되는 반복 밴드의 루프문
			//데이터 fetch
			setFieldVariableFrompos_receipt();	//RecordSet pos_receipt에서 필드 값 설정

			if(bIsFirst){   //밴드 호출 후 루프 내에서 한번 만 실행되는 코드
			}   //end if(bIsFirst)

			//데이터 변수 할당
			sheet10_B2=pos_receipt_RECEIPT;
			...
			if(bIsPrint){   //감추기 속성이 지정되지 않으면 bIsPrint는 true
				...
				String []sheet10_B2_Split=new String[2];
				do{
					...
					nAutoHeight_test[0]=21;
					nAutoHeight_test[0]=getPrintHeightEx(sheet10_B2, sheet10_B2_Split, charWidth, 9, 268, nYEndPos-nYCurPos-2, nAutoHeight_test[0], 16, 2, "cell_style_1");
					nBandHeight=nAutoHeight_test[0];
					if(!sheet10_B2_Split[0].equals(isEmpty)){
						sheet10_B2=sheet10_B2_Split[0];
					}
					...
					//스크립트 출력 함수 호출
					writeScripttest(bSplitLoopPrint);

					//페이지 연결 출력 및 가상병합 플래그 Reset
					if(sheet10_B2_Split[0].equals(isEmpty)){
						bSplitLoop=false;
					}else{
						sheet10_B2=sheet10_B2_Split[1];
	...
	//----bIsPrint : 감추기 속성 설정 여부. true면 출력, false면 AI 스크립트를 생성하지 않음.
	//----nRepeatCnt : 부모 반복 밴드 수행 횟수와 연동 옵션 설정시 void 밴드가 수행할 반복 횟수 전달
	private void getScriptFromtest2(boolean bIsFirst, boolean bIsPrint) throws Exception{
		//지역 변수 선언 및 초기화
		int nBandHeight=21;    //밴드 출력 높이
	...
	//-----스크립트 출력 함수-----
	//----vv3 스크립트 문 출력 함수----
	private void writeScriptvv3( ) throws Exception{
		//스크립트문 생성
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_B1"), nYCurPos+mapTop.get("sheet10_B1"), mapWidth.get("sheet10_B1"), mapHeight.get("sheet10_B1"), sheet10_B1, "sheet10_B1", mapStyle.get("sheet10_B1"));
	}

	//----test 스크립트 문 출력 함수----
	private void writeScripttest(boolean bSplitLoopPrint) throws Exception{
		//스크립트문 생성
	...
	//----test2 스크립트 문 출력 함수----
	private void writeScripttest2( ) throws Exception{
		//스크립트문 생성
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_B4"), nYCurPos+mapTop.get("sheet10_B4"), mapWidth.get("sheet10_B4"), mapHeight.get("sheet10_B4"), "", "sheet10_B4", mapStyle.get("sheet10_B4"));
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_C4"), nYCurPos+mapTop.get("sheet10_C4"), mapWidth.get("sheet10_C4"), mapHeight.get("sheet10_C4"), "", "sheet10_C4", mapStyle.get("sheet10_C4"));
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_D4"), nYCurPos+mapTop.get("sheet10_D4"), mapWidth.get("sheet10_D4"), mapHeight.get("sheet10_D4"), "", "sheet10_D4", mapStyle.get("sheet10_D4"));
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_E4"), nYCurPos+mapTop.get("sheet10_E4"), mapWidth.get("sheet10_E4"), mapHeight.get("sheet10_E4"), "", "sheet10_E4", mapStyle.get("sheet10_E4"));
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_F4"), nYCurPos+mapTop.get("sheet10_F4"), mapWidth.get("sheet10_F4"), mapHeight.get("sheet10_F4"), "", "sheet10_F4", mapStyle.get("sheet10_F4"));
	}
	...
	private void initSQLConst() throws Exception {
		stat_pos_receipt = initializer.configPreparedStatement("sql_pos_receipt", mapParam, conn_retail, null);
	}

	//-----ResultSet 객체에서 필드 값 설정 함수-----
	//ResultSet pos_receipt 객체에서 필드 값 설정 함수
	private void setFieldVariableFrompos_receipt() throws Exception{
		int size;
		Clob clob;

		clob=pos_receipt.getClob(1);
		if(clob==null){
			size=0;
		}
		else{
		...
		if(size!=0){
			pos_receipt_RECEIPT=clob.getSubString(1, size);
		}
		else{
			pos_receipt_RECEIPT="";
		}
		pos_receipt_TIME = GetB(pos_receipt.getString(2));
	}

	//ResultSet pos_receipt 객체의 필드 값 초기화 함수
	private void setFieldInitFrompos_receipt(){
		pos_receipt_RECEIPT="";
		pos_receipt_TIME = "";
	}
	...
	private void getScriptHeader(String paper, String size, String value, String orient, String margin) throws Exception{
		...
		manager.scriptWrite("AIReport", "scriptVersion|docVersion|style|title", "32|16|posClosedReceiptPrint.style|보고서 (2017/4/24 11:10:41)", false);
	}
	...
	public void setCellInfoMap() throws Exception {
		...
		setCellInfoMap_Internal("sheet10_B1", 127, 0, 284, 22, "cell_style_0");
		setCellInfoMap_Internal("sheet10_B2", 127, 0, 284, nAutoHeight_test[0], "cell_style_1");
		setCellInfoMap_Internal("sheet10_B4", 127, 0, 56, 21, "cell_style_2");
		setCellInfoMap_Internal("sheet10_C4", 183, 0, 56, 21, "cell_style_2");
		setCellInfoMap_Internal("sheet10_D4", 239, 0, 56, 21, "cell_style_2");
		setCellInfoMap_Internal("sheet10_E4", 295, 0, 56, 21, "cell_style_2");
		setCellInfoMap_Internal("sheet10_F4", 351, 0, 60, 21, "cell_style_2");
	}
}
%>
