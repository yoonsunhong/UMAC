...
public void jspInit() {
	...
	sql_credit.append("    ,       CLSD.DPOT_AMT AS DPOT_DTL_AMT ");
	...
	sql_credit.append("    ,       SUBSTR(CLSD.RCP_DT,0,4)||'-'||SUBSTR(CLSD.RCP_DT,5,2)||'-'||SUBSTR(CLSD.RCP_DT,7,2) AS RCP_DT_ ");
	sql_credit.append("    FROM ");
	sql_credit.append("            CS_LEDGER_SLIP_DTL  CLSD ");
	sql_credit.append("    ,       CS_CUST             CC ");
	sql_credit.append("    WHERE   1=1 ");
	sql_credit.append("    AND     CLSD.CORP_CODE      =   CC.CORP_CODE(+) ");
	sql_credit.append("    AND     CLSD.CUST_NO        =   CC.CUST_NO(+) ");
	sql_credit.append(" ");
	sql_credit.append("    AND     CLSD.CORP_CODE       =   '^P_CORP_CODE^' ");
	sql_credit.append("    AND     CLSD.DPOT_STR_CODE        =   NVL('^P_STR_CODE^',CLSD.DPOT_STR_CODE) ");
	sql_credit.append("    AND     CLSD.DPOT_FLAG      =   NVL('^P_DPOT_FLAG^',CLSD.DPOT_FLAG) ");
	sql_credit.append("    AND     CLSD.RCP_DT          BETWEEN   replace('^P_BUSI_SD^','-','') AND  replace('^P_BUSI_ED^','-','') ");
	sql_credit.append("    AND     CLSD.CUST_NO         =   NVL('^P_CUST_NO^',CLSD.CUST_NO) ");
	sql_credit.append("    AND     CLSD.DPOT_FLAG      != '04' ");
	sql_credit.append("    ORDER BY CC.CUST_NO,CLSD.RCP_DT ");
...
class ReportHandler{
	...
	private void getScriptFromfv1(boolean bIsPrint) throws Exception{
		...
		while(credit.next()){   //자동으로 생성되는 반복 밴드의 루프문
			...
			//데이터 변수 할당
			if(!sheet20_A3.equals(credit_ACCT_DEPT)){
				if(!bIsFirst){
			...
			if(!sheet20_B3.equals(credit_CUST_NO)){
				if(!bIsFirst){
			...
			if(!sheet20_C3.equals(credit_CUST_NAME)){
				if(!bIsFirst){
	...
	private void getScriptHeader(String paper, String size, String value, String orient, String margin) throws Exception{
		...
		manager.scriptWrite("AIReport", "scriptVersion|docVersion|style|title", "32|10|creditDpotDtlStatePrint.style|보고서 (2017/5/11 9:30:50)", false);
	}
	...
	public void setCellInfoMap() throws Exception {
		...
		setCellInfoMap_Internal("sheet20_A3", 0, -1, 47, -1, "cell_style_9");
		setCellInfoMap_Internal("sheet20_B3", 47, -1, 47, -1, "cell_style_10");
		setCellInfoMap_Internal("sheet20_C3", 94, -1, 71, -1, "cell_style_11");
