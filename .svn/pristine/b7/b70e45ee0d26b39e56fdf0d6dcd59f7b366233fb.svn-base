...
public void jspInit() {
	...
	//sql_EMATE_TOTAL_AMT
	StringBuilder sql_EMATE_TOTAL_AMT = new StringBuilder();
	sql_EMATE_TOTAL_AMT.setLength(0);
	sql_EMATE_TOTAL_AMT.append("                SELECT ");
	sql_EMATE_TOTAL_AMT.append("                            NUM_TO_HANGUL(NVL(SUM(CASE ");
	sql_EMATE_TOTAL_AMT.append("                                WHEN CED.TAX_GB = '1' ");
	sql_EMATE_TOTAL_AMT.append("                                    THEN  (CED.EMATE_SPRC * CED.ORD_QTY) - TRUNC( (CED.EMATE_SPRC * CED.ORD_QTY) / 11, 0 ) ");
	sql_EMATE_TOTAL_AMT.append("                                ELSE CED.EMATE_SPRC * CED.ORD_QTY ");
	sql_EMATE_TOTAL_AMT.append("                             END ),0)) AS EMATE_AMT_HAN ");
	sql_EMATE_TOTAL_AMT.append(" ");
	sql_EMATE_TOTAL_AMT.append("                FROM    CS_EMATE_DTL CED ");
	sql_EMATE_TOTAL_AMT.append("                ,            CD_PRODUCT_CMN CPC ");
	sql_EMATE_TOTAL_AMT.append("                WHERE  CED.CORP_CODE = '^P_CORP_CODE^' ");
	sql_EMATE_TOTAL_AMT.append("                AND       CED.EMATE_DT =  REPLACE('^P_EMATE_DT^', '-') ");
	sql_EMATE_TOTAL_AMT.append("                AND       CED.EMATE_NO = '^P_EMATE_NO^' ");
	sql_EMATE_TOTAL_AMT.append("                AND       CED.SCAN_CODE = CPC.SCAN_CODE(+) ");
	sql_EMATE_TOTAL_AMT.append("                AND       CPC.USE_YN <> 'N' ");
	sql_EMATE_TOTAL_AMT.append("                ORDER BY CED.SEQ ");
	initializer.prepareQuery("sql_EMATE_TOTAL_AMT", sql_EMATE_TOTAL_AMT.toString());
}
...
class ReportHandler{
	...
	private java.sql.PreparedStatement stat_EMATE_TOTAL_AMT;		//ResultSet EMATE_TOTAL_AMT 객체를 생성하기 위한 PreparedStatement 객체
	private ResultSet EMATE_TOTAL_AMT;		//ResultSet EMATE_TOTAL_AMT 객체
	...
	private StringBuilder sql_EMATE_TOTAL_AMT = new StringBuilder();		//ResultSet EMATE_TOTAL_AMT 객체를 생성하기 위한 쿼리문 저장
	...
	//ResultSet EMATE_TOTAL_AMT 객체에서 필드 값을 받는 쿼리 필드 변수
	private String EMATE_TOTAL_AMT_EMATE_AMT_HAN;
	...
	void service(JspWriter out) throws Exception{
		try{    //try 구문
			...
			//DataSet Close
			cleanup(EMATE_TOTAL_AMT, stat_EMATE_TOTAL_AMT, null, null);
	...
	private void getScriptFromsh_vb(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			...
			//ResultSet EMATE_TOTAL_AMT 객체의 생성
			if(EMATE_TOTAL_AMT==null){
				EMATE_TOTAL_AMT = stat_EMATE_TOTAL_AMT.executeQuery();
			}else{
				EMATE_TOTAL_AMT.beforeFirst();
			}
			...
			if(EMATE_TOTAL_AMT.next()){
				setFieldVariableFromEMATE_TOTAL_AMT();	//ResultSet EMATE_TOTAL_AMT에서 필드 값 설정
			}else{
				setFieldInitFromEMATE_TOTAL_AMT();	//ResultSet EMATE_TOTAL_AMT의 필드 값 초기화
			}
			...
			sheet10_A17="견적금액 : ";
			sheet10_D17 = EMATE_TOTAL_AMT_EMATE_AMT_HAN;
			sheet10_G17="(VAT 별도)";
			...
			nAutoHeight_sh_vb[4]=getPrintHeight(sheet10_B5, charWidth, 9, 209, 12, nAutoHeight_sh_vb[4], 5, 2, "cell_style_10");
	...
	private void writeScriptsh_vb( ) throws Exception{
		...
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_H17"), nYCurPos+mapTop.get("sheet10_H17"), mapWidth.get("sheet10_H17"), mapHeight.get("sheet10_H17"), "", "sheet10_H17", mapStyle.get("sheet10_H17"));
		...
		objectWrite(DRAW_OBJECT, nXCurPos+6, nYCurPos+3, 140, 21, "", "AI_IMAGE_OBJ18", "draw_style_1");
	}
	...
	private void initSQLConst() throws Exception {
		...
		stat_EMATE_TOTAL_AMT = initializer.configPreparedStatement("sql_EMATE_TOTAL_AMT", mapParam, conn_retail, null);
	}
	...
	//ResultSet EMATE_TOTAL_AMT 객체에서 필드 값 설정 함수
	private void setFieldVariableFromEMATE_TOTAL_AMT() throws Exception{
		EMATE_TOTAL_AMT_EMATE_AMT_HAN = GetB(EMATE_TOTAL_AMT.getString(1));
	}

	//ResultSet EMATE_TOTAL_AMT 객체의 필드 값 초기화 함수
	private void setFieldInitFromEMATE_TOTAL_AMT(){
		EMATE_TOTAL_AMT_EMATE_AMT_HAN = "";
	}
	...
	private void getScriptHeader(String paper, String size, String value, String orient, String margin) throws Exception{
		...
		manager.scriptWrite("AIReport", "scriptVersion|docVersion|style|title", "32|20|estimatePrint.style|보고서 (2017/3/9 13:52:2)", false);
	}
	...
	public void setCellInfoMap() throws Exception {
		...
		setCellInfoMap_Internal("sheet10_A17", 0, nAutoHeight_sh_vb[15], 180, nAutoHeight_sh_vb[16]-nAutoHeight_sh_vb[15], "cell_style_35");
		setCellInfoMap_Internal("sheet10_D17", 180, nAutoHeight_sh_vb[15], 142, nAutoHeight_sh_vb[16]-nAutoHeight_sh_vb[15], "cell_style_36");
		setCellInfoMap_Internal("sheet10_G17", 322, nAutoHeight_sh_vb[15], 96, nAutoHeight_sh_vb[16]-nAutoHeight_sh_vb[15], "cell_style_37");
		setCellInfoMap_Internal("sheet10_H17", 418, nAutoHeight_sh_vb[15], 126, nAutoHeight_sh_vb[16]-nAutoHeight_sh_vb[15], "cell_style_38");
		setCellInfoMap_Internal("sheet10_A18", 0, nAutoHeight_sh_vb[16], 544, nAutoHeight_sh_vb[17]-nAutoHeight_sh_vb[16], "cell_style_39");
		...
		setCellInfoMap_Internal("sheet20_A1", 0, 0, 211, 20, "cell_style_40");
		setCellInfoMap_Internal("sheet20_B1", 211, 0, 31, 20, "cell_style_41");
		setCellInfoMap_Internal("sheet20_C1", 242, 0, 48, 20, "cell_style_41");
		setCellInfoMap_Internal("sheet20_D1", 290, 0, 31, 20, "cell_style_41");
		setCellInfoMap_Internal("sheet20_E1", 321, 0, 31, 20, "cell_style_41");
		setCellInfoMap_Internal("sheet20_F1", 352, 0, 48, 20, "cell_style_41");
		setCellInfoMap_Internal("sheet20_G1", 400, 0, 48, 20, "cell_style_42");
		setCellInfoMap_Internal("sheet20_H1", 448, 0, 48, 20, "cell_style_42");
		setCellInfoMap_Internal("sheet20_I1", 496, 0, 48, 20, "cell_style_43");
		setCellInfoMap_Internal("sheet20_A2", 0, 0, 211, 20, "cell_style_44");
		setCellInfoMap_Internal("sheet20_B2", 211, 0, 31, 20, "cell_style_45");
		setCellInfoMap_Internal("sheet20_C2", 242, 0, 48, 20, "cell_style_45");
		setCellInfoMap_Internal("sheet20_D2", 290, 0, 31, 20, "cell_style_46");
		setCellInfoMap_Internal("sheet20_E2", 321, 0, 31, 20, "cell_style_46");
		setCellInfoMap_Internal("sheet20_F2", 352, 0, 48, 20, "cell_style_46");
		setCellInfoMap_Internal("sheet20_G2", 400, 0, 48, 20, "cell_style_46");
		setCellInfoMap_Internal("sheet20_H2", 448, 0, 48, 20, "cell_style_46");
		setCellInfoMap_Internal("sheet20_I2", 496, 0, 48, 20, "cell_style_47");
		setCellInfoMap_Internal("sheet20_A3", 0, 0, 400, 20, "cell_style_48");
		setCellInfoMap_Internal("sheet20_G3", 400, 0, 48, 20, "cell_style_49");
		setCellInfoMap_Internal("sheet20_H3", 448, 0, 48, 20, "cell_style_49");
		setCellInfoMap_Internal("sheet20_I3", 496, 0, 48, 20, "cell_style_50");
		setCellInfoMap_Internal("sheet20_A4", 0, 0, 211, 1, "cell_style_51");
		setCellInfoMap_Internal("sheet20_B4", 211, 0, 31, 1, "cell_style_52");
		setCellInfoMap_Internal("sheet20_C4", 242, 0, 48, 1, "cell_style_52");
		setCellInfoMap_Internal("sheet20_D4", 290, 0, 31, 1, "cell_style_52");
		setCellInfoMap_Internal("sheet20_E4", 321, 0, 31, 1, "cell_style_52");
		setCellInfoMap_Internal("sheet20_F4", 352, 0, 48, 1, "cell_style_53");
		setCellInfoMap_Internal("sheet20_G4", 400, 0, 48, 1, "cell_style_53");
		setCellInfoMap_Internal("sheet20_H4", 448, 0, 48, 1, "cell_style_53");
		setCellInfoMap_Internal("sheet20_I4", 496, 0, 48, 1, "cell_style_54");
		setCellInfoMap_Internal("sheet20_A5", 0, 1, 544, 20, "cell_style_55");
		setCellInfoMap_Internal("sheet20_A6", 0, 21, 544, 20, "cell_style_56");
	}
}
%>
