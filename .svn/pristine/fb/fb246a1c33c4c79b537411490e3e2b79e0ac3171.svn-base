...
'<rMateGrid>\
	...
	<DataGrid id="dg1" sortableColumns="true"  verticalAlign="middle"  showDataTips="true"   itemRenderer="EditableIconItem"  showDeletedRows="true" >\
			<groupedColumns>\
				...
				<DataGridColumn id="INV_QTY" 		dataField="INV_QTY" 		headerText="'+actualQuantity+'"   		textAlign="right" 	width="80"	sortable="false" formatter="{numfmt}" />\
				...
				<DataGridColumn id="SPRC" 			dataField="SPRC"  			headerText="'+wprc+'"   				textAlign="right" 	width="100" sortable="false" formatter="{numfmt}" />\
				<DataGridColumn id="STOCK_PRICE" 	dataField="STOCK_PRICE"  	headerText="'+stockAmount+'"   			textAlign="right" 	width="120" sortable="false" formatter="{numfmt}" />\
				...
				<DataGridColumn id="SURVEY_NM"		dataField="SURVEY_NM"  		headerText="'+investigator+'" 			textAlign="center" 	width="80"  sortable="false" />\
				<DataGridColumn id="UEMP_NM" 		dataField="UEMP_NM"  		headerText="'+modifier+'" 				textAlign="center" 	width="80"  sortable="false" />\
	...
	'<rMateGrid>\
		...
		<NumberFormatter id="numfmt" useThousandsSeparator="true"/>\
			<DataGrid id="dg1" sortableColumns="true" horizontalScrollPolicy="auto"    selectionMode="multipleCells"       >\
				<columns>\
					<DataGridColumn id="CD"   			dataField="CD" 					headerText="'+codeTop+'" 			textAlign="center" labelJsFunction="labelFunc" 		width="80"/>\
...
//		height :"50%",
	...
	jQuery.ajax({
	    ...
	    async:false,
	    success : function(data) {
	    	$("#P_CLS_CODE").append('<option value="00">'+all+'</option>'); 
			for(var i = 0; i < data.length; i++){
				 $("#P_CLS_CODE").append('<option value="'+ data[i].CD_ID +'">'+ data[i].CD_NM +'</option>'); 
		   	}
	    },
	    complete : function(data) {
	    },
	...
	$("#P_POP_SCAN_CODE").on('keyup', function(){
		...
		var P_INV_INSP_GB = $("#P_INV_INSP_GB").val();
	...
	//숫자만 입력
	CommonJs.addInputHandler({input:$("#P_POP_SCAN_CODE"), dataType:"N", maxlength:13});
	CommonJs.addInputHandler({input:$("#P_POP_INV_QTY"), dataType:"N", maxlength:7});
...
function btn_del(){
	...
	if($("#CFM_FLAG").val()=="2" || $("#CFM_FLAG").val()=="1"){
	 ...
	 if($("#P_INV_INSP_SCHD_ID").val() == ""){
		...
		jQuery.ajax({
		    ...
		    success : function(data) {
		    	...
		    	$("#P_INV_INSP_GB").val(data.INV_INSP_GB);
		    },
		    complete : function(data) {
		    },
		    error : function(xhr, status, error) {
...
//그리드 너비 제어
$(document).ready(function (){
	$("#gridHolder1, #gridHolder2").height( $(window).height() - 103 );
	$(".ui-dialog-titlebar").text("실사재고등록 미리보기");
...
//탭1 그리드 확정(IV_ITEM_PHY_STOCK_ADD--->IV_ITEM_PHY_STOCK) 
function tab1_submitCheck(){
	...
	jQuery.ajax({ 
		...
		success:function(data){  
			...
			{ 
				...
				var SURVEY_NM 				= xmlDoc.createElement('SURVEY_NM');  
				...
				var UEMP_NM 					= xmlDoc.createElement('UEMP_NM');
				...
				SURVEY_NM.appendChild(			xmlDoc.createTextNode(	data[i].SURVEY_NM		));
				...
				UEMP_NM.appendChild(				xmlDoc.createTextNode(	data[i].UEMP_NM			));
				...
				xmlDoc.getElementsByTagName("GRIDROW")[0].appendChild(SURVEY_NM);   
				...
				xmlDoc.getElementsByTagName("GRIDROW")[0].appendChild(UEMP_NM);   
...
function downSmapleExcel(){
	location.href="/resources/js/page/stock/stockReal/SAMPLE.xlsx";
}
        ...
        var options = {
            success : function(data) {
                ...
                if(data.VALID_YN == "Y"){
                	...
                	//엑셀 업로드 데이터중 '점포코드', '스캐닝코드', '조사수량', '매장구분'의\n데이터 형식이 잘못 되었습니다.\n데이터를 올바르게 수정 한 뒤 엑셀업로드를 하세요.
                	alert(stockRealMent14+"\n"+stockRealMent15+"\n"+stockRealMent16);
                }
...
//	}
	...
	if($("#VALID_YN").val()=="N"){
		//실사재고등록 데이터중 '점포코드', '스캐닝코드', '조사수량', '매장구분'의\n데이터 형식이 잘못 되었습니다.\n데이터를 올바르게 수정 한 뒤 다시 시도하여 주세요.
		alert(stockRealMent14+"\n"+stockRealMent15+"\n"+stockRealMent16);
		return;
	}
	...
	for(var i = 0; i < rowCnt.length ; i++)
	   {  // 발주 삭제 - 슬립넘버를 xml 로  만든다.  
	         ...
	         SEQ.appendChild( xmlDoc.createTextNode(        gridRoot1.getItemFieldAt( i , "SEQ")     )   );   
	          ...
	          var INV_QTY      = xmlDoc.createElement('INV_QTY');   
	          INV_QTY.appendChild( xmlDoc.createTextNode(        gridRoot1.getItemFieldAt( i , "INV_QTY")     )   );   
	          xmlDoc.getElementsByTagName("GRIDROW")[0].appendChild(      INV_QTY       );   
	          ...
	          var INV_FLAG_CODE      = xmlDoc.createElement('INV_FLAG_CODE');   
	          INV_FLAG_CODE.appendChild( xmlDoc.createTextNode(        gridRoot1.getItemFieldAt( i , "INV_FLAG_CODE")     )   );   
	          xmlDoc.getElementsByTagName("GRIDROW")[0].appendChild(      INV_FLAG_CODE       );   
	          
	          var CFM_FLAG_CODE      = xmlDoc.createElement('CFM_FLAG_CODE');   
	          CFM_FLAG_CODE.appendChild( xmlDoc.createTextNode(        gridRoot1.getItemFieldAt( i , "CFM_FLAG_CODE")     )   );   
	          xmlDoc.getElementsByTagName("GRIDROW")[0].appendChild(      CFM_FLAG_CODE       );   
...
//	//실사재고 조회(확정)-탭2
//	jQuery.ajax({ 
//	    url:"/getStockRealDataCheckList.do",         
//	    type:"POST",
//		datatype:"xml",
//		async:false,
//		data: loadData, 
...
function ExcelDownload1() {
	...
	dataGrid1.exportFileName = "실사재고등록_"+$("#P_STR_NAME option:selected").text()+"_"+$("#P_INV_INSP_SCHD_ID").val()+".xlsx";
...
function pop1_save() {
  ...
  {   parser = new DOMParser();
	...
	SPRC.appendChild( 				xmlDoc.createTextNode(	$("#P_POP_SPRC").val().replace(',','')			));
	//재고금액 산출 (단가 * 재고조사 수량)
	var inv_qty = $("#P_POP_INV_QTY").val();
	var sprc = $("#P_POP_SPRC").val().replace(',','');
	...
	INV_FLAG.appendChild(				xmlDoc.createTextNode(	"수기"								));
...
function ExcelDownload2() {
	...
	dataGrid2.exportFileName = "실사재고등록(확정)_"+$("#P_STR_NAME option:selected").text()+"_"+$("#P_INV_INSP_SCHD_ID").val()+".xlsx";
