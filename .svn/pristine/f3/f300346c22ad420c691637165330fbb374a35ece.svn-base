...
'<rMateGrid>\
	...
	<DataGrid id="dg1"  sortableColumns="true"  editable="true" horizontalScrollPolicy="auto" showDeletedRows="true"    selectionMode="multipleCells"   doubleClickEnabled="false" >\
		<columns>\
			...
			<DataGridColumn  id="ORD_QTY"		 dataField="ORD_QTY"     	headerText="발주수량"		editable="false" 		textAlign="right"	 width="70"    maxChars="6" type="int"     formatter="{numfmt}"  showEditableIcon="always" itemRenderer="EditableIconItem" 	/>\
			<DataGridColumn  id="TAX_GB"		 dataField="TAX_GB"     	headerText="TAX_GB"		visible="false" />\
			<DataGridColumn  id="QTY"       	 dataField="QTY"       		headerText="대입수량"		editable="true"			textAlign="right"    width="98"  showEditableIcon="always"   itemRenderer="EditableIconItem"	    formatter="{numfmt}"		/>\
			...
			<DataGridColumn  id="WAMT"		 	 dataField="WAMT"       	headerText="매입단가"     editable="true"			textAlign="right"	 width="98"    showEditableIcon="always"   itemRenderer="EditableIconItem"	   formatter="{numfmt}" 	/>\
			<DataGridColumn  id="WPRC"		 	 dataField="WPRC"       	headerText="대입공급가"    editable="true"			textAlign="right"	 width="98"    showEditableIcon="always"   itemRenderer="EditableIconItem"	   formatter="{numfmt}" 	/>\
			<DataGridColumn  id="WVAT"   	 	 dataField="WVAT"    		headerText="대입부가세" 	editable="true"			textAlign="right"	 width="108"   showEditableIcon="always"   itemRenderer="EditableIconItem"	   formatter="{numfmt}" 	/>\
			<DataGridColumn  id="SPRC"   	     dataField="SPRC"    	    headerText="대입매가단가"   editable="false"		textAlign="right"	 width="98"     formatter="{numfmt}" 	/>\
			<DataGridColumn  id="WPRC_TOT"		 dataField="WPRC_TOT"       headerText="대입합계금액"   editable="true"		 	textAlign="right"	 width="100"     showEditableIcon="always"   itemRenderer="EditableIconItem"		 formatter="{numfmt}" 	/>\
	...
	<DateFormatter id="datefmt" formatString="YYYY-MM-DD"/>\
		...
		<DataGrid id="dg1" sortableColumns="true"  dragSelectable="true"  editable="true" horizontalScrollPolicy="auto" showDeletedRows="true"     doubleClickEnabled="false" selectionMode="multipleRows" >\
			<columns>\
				...
				<DataGridColumn  id="UEMP_NO"	  	  dataField="UEMP_NO"    		headerText="최종수정자"  	visible="true"  		editable="false"		textAlign="center"   width="88"      	/>\
...
//    orderStoreHeader  =  "<GRIDLIST>"+getXmlString(   xmlDoc   )+"</GRIDLIST>" ; 
	...
	jQuery.ajax({ 
		...
		,   "INOUT_GB"       : gridRoot2.getItemFieldAt( selectedIndex , "INOUT_GB") 
	    }, 
...
function btn_search(){
	...
	// 대출입 헤더 리스트 보여주기 
	jQuery.ajax({ 
	    url:"/inOutCenterHeadSearch.do",         //  purchHeadSearch 
	    type:"POST",
		datatype:"json",
		async:false,
...
{ 
	...
	jQuery.ajax({ 
	    url:"/inOutCenterDetailInfo.do",     // purchDetailInfo.do        
	    type:"POST",
		datatype:"xml",
		async:false,
		...
		success:function(data){  
			...
			{ 
				...
				var INOUT_CFM_QTY	= xmlDoc.createElement('INOUT_CFM_QTY');
				...
				INOUT_CFM_QTY.appendChild(	xmlDoc.createTextNode(	data[i].INOUT_CFM_QTY	)	); 
				
				//대입합계금액
				var WPRC_TOT_VAL = parseInt(	(parseFloat(data[i].WPRC) + parseFloat(data[i].WVAT)) * parseFloat(data[i].INOUT_CFM_QTY ) ) * 1;
				
				WPRC_TOT.appendChild(		xmlDoc.createTextNode(	 WPRC_TOT_VAL	)	); 
				WAMT.appendChild(			xmlDoc.createTextNode(	( parseFloat(data[i].WPRC)  + parseFloat(data[i].WVAT) )  	)	); 
...
{   
	...
	var WSPRC_CFM_TAX_ADD 	= 0;	// 과세 확정수량 + 대출원가
	var WSPRC_TAX_FREE 		= 0;   	// 면세 대출원가
	var WSPRC_CFM_TAX_FREE	= 0;	// 면세 확정수량 + 대출원가
	var WVAT  				= 0;   	// 부가세
	var CFM_WVAT			= 0;	//확정수량 + 부가세
	var SPRC  				= 0;   	// 매가
	...
	{   
		...
		QTY 	= gridRoot1.getItemFieldAt(i, "QTY");		//대입수량
		CMF_QTY = gridRoot1.getItemFieldAt(i, "CFM_QTY");	//대입확정수량

		//대출원가 - 과세
		if(gridRoot1.getItemFieldAt(i, "TAX_GB") == "1")
		{
			WSPRC_TAX_ADD 		= parseFloat(WSPRC_TAX_ADD) 	+ (QTY     * parseFloat(gridRoot1.getItemFieldAt(i, "WPRC")));
			WSPRC_CFM_TAX_ADD 	= parseFloat(WSPRC_CFM_TAX_ADD) + (CMF_QTY * parseFloat(gridRoot1.getItemFieldAt(i, "WPRC")));
		}
		//대출원가 - 면세
		else
		{
			WSPRC_TAX_FREE 		= parseFloat(WSPRC_TAX_FREE) 	 + (QTY     * parseFloat(gridRoot1.getItemFieldAt(i, "WPRC")));
			WSPRC_CFM_TAX_FREE 	= parseFloat(WSPRC_CFM_TAX_FREE) + (CMF_QTY * parseFloat(gridRoot1.getItemFieldAt(i, "WPRC")));
		} 
		WVAT 		 = parseFloat(WVAT) 		+ (QTY * parseFloat(gridRoot1.getItemFieldAt(i, "WVAT")));
		CFM_WVAT	 = parseFloat(CFM_WVAT) 	+ (CMF_QTY * parseFloat(gridRoot1.getItemFieldAt(i, "WVAT")));
	...
	// 구입 금액합계
	$("#TOT_PRICE").val(parseFloat(WSPRC_CFM_TAX_FREE) + parseFloat(WSPRC_CFM_TAX_ADD) + parseFloat(CFM_WVAT));
...
//								, "PUR_WAMT"  ); 
	...
	if(   dataField == "WAMT"  ) {  
		...
		var QTY      =  gridRoot1.getItemFieldAt( rowIndex , "QTY"   )   ;  
...
//		gridRoot1.setItemFieldAt( parseFloat( priceVat[2] )   * parseFloat( DEC_QTY	) 	, rowIndex, "PUR_WVAT_TOT");  
		...
		// 매입원가총금액 =  수량 * 매입원가금액
		// PUR_WAMT_TOT =  QTY * PUR_WAMT 
		 
		gridRoot1.setItemFieldAt( parseInt( parseFloat(newValue) * parseFloat(QTY)     ) *1
								, rowIndex 
								, "WPRC_TOT"  ); 
...
//		gridRoot1.setItemFieldAt( gridRoot1.getItemFieldAt( rowIndex , "PUR_WVAT"  ) , rowIndex  , "OLD_PUR_WVAT"  ); 
	...
	// 공급가 변경시 : 매입단가(WAMT) = 공급가 + 부가세    ,      합계금액(WAMT_TOT)  = (공급가 + 부가세 ) * 확정수량
	if(   dataField == "WPRC"  ) 
	{
		var WPRC = gridRoot1.getItemFieldAt( rowIndex , "WPRC") ;    // 공급가
		var WVAT = gridRoot1.getItemFieldAt( rowIndex , "WVAT") ;    // 부가세
		var QTY  = gridRoot1.getItemFieldAt( rowIndex , "QTY") ;     // 확정수량 
		
		gridRoot1.setItemFieldAt(  parseFloat( WPRC ) + parseFloat( WVAT )            , rowIndex, "WAMT");        // 매입단가
//		gridRoot1.setItemFieldAt(  parseInt (  (parseFloat(WPRC) + parseFloat(WVAT)) *  parseFloat(QTY)  )*1 , rowIndex, "WAMT_TOT");    // 합계금액
	
		gridRoot1.setItemFieldAt(   parseFloat(newValue) *  parseFloat( QTY)   , rowIndex, "WPRC_TOT");    // 공급가 * 수량금액
		gridRoot1.setItemFieldAt( "U" , rowIndex  , "CRUD"  ); 
		setTotalSum();
	}
	...
	if(   dataField == "WVAT"  ) 
	{
			...
			if( TAX_GB == "2")  //면세
			{
				...
				gridRoot1.setItemFieldAt( "0"   , rowIndex, "WVAT");    
				return;
			} else { 
				var WPRC = gridRoot1.getItemFieldAt( rowIndex , "WPRC") ;    // 공급가
				var WVAT = gridRoot1.getItemFieldAt( rowIndex , "WVAT") ;    // 부가세
				...
				gridRoot1.setItemFieldAt(  parseFloat( WPRC ) + parseFloat( WVAT )                , rowIndex, "WAMT");        // 매입단가
				gridRoot1.setItemFieldAt( parseInt(  (parseFloat(WPRC) + parseFloat(WVAT)) *  parseFloat( QTY ) ) *1  , rowIndex, "WPRC_TOT");    // 합계금액
...
//				gridRoot1.setItemFieldAt(   parseFloat(newValue) *  parseFloat(DEC_QTY)   , rowIndex, "PUR_WVAT_TOT");    // 공급가 * 수량금액
	...
	if(   dataField == "WPRC_TOT"  ) {           // PUR_WAMT_TOT  
		 
		if( newValue == "0" ||  $.trim( newValue ) == "")
		{
			gridRoot1.setItemFieldAt(  oldValue , rowIndex  , "WPRC_TOT"  ); 
			alert("합계금액을 입력해 주세요."); 
			return;
		}
		...
		var QTY1  =  gridRoot1.getItemFieldAt( rowIndex , "QTY" ) ;  
		var priceVat =  calPriceVat(  parseFloat(newValue / QTY1 )  , TAX_GB1  ); 
		 
		gridRoot1.setItemFieldAt(  priceVat[1].toFixed(2)     , rowIndex  , "WPRC"  ); 
		gridRoot1.setItemFieldAt(  priceVat[2]                , rowIndex  , "WVAT"  );  
		gridRoot1.setItemFieldAt(  parseFloat(priceVat[1].toFixed(2) ) + parseFloat( priceVat[2])  , rowIndex  , "WAMT"  );  
