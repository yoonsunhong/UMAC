	...
	'<rMateGrid>\
		...
		<DataGrid id="dg1"  sortableColumns="true"  editable="true" horizontalScrollPolicy="auto" showDeletedRows="true"     doubleClickEnabled="false" selectionMode="multipleRows" >\
			<columns>\
				...
				<DataGridColumn  id="VEN_NAME"	      dataField="VEN_NAME"    	headerText="매입처"           editable="false"		textAlign="left"   width="120"      	/>\
...
//	
//
//	if( $("#SESSION_ROLE_ID").val()    == "ROLE009")  // 바이어 로그인 확정수량 
//	{ 
//		return false;
//	} else {
//		return false;
...
//    xmlDoc.getElementsByTagName("GRIDROW")[0].appendChild(	   ITM_GB	     );           
	...
	jQuery.ajax({ 
	    url:"/orderStoreProductRegisterR2.do",          
	    type:"POST",
		datatype:"xml",
		async:true,
...
function btn_clear(){
	...
	// 배송구분 초기화 후,  다시 그려주고, R1 을 뺀다. 
//	$('#ROUTE_GB').find('option').remove() ; 
//	getCommonCodeSelectBoxList("ROUTE_GB"  ,   "ROUTE_GB"	);  
//	$("#ROUTE_GB  option[value='R1']").remove();    // 배송구분에서 R2/R3 만 보여줌.
	...
	if(   dataField == "SCAN_CODE"  ) {  
		...
		if( typeof SCAN_CODE   != 'undefined' )
		{
			...
			{ 	
				...
				jQuery.ajax({ 
				    url:"/orderStoreProductSelectR2.do",              
				    type:"POST",
					datatype:"json",
					async:false,
