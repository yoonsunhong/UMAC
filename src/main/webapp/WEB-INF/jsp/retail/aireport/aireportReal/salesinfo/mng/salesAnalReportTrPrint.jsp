...
public void jspInit() {
	...
	sql_sale_state.append("SELECT  SH.POS_NO ");
	sql_sale_state.append(",  SH.STR_CODE ");
	sql_sale_state.append(", SH.TRXN_NO ");
	sql_sale_state.append(",SI.SCAN_CODE ");
	sql_sale_state.append(",SI.SALE_PRC ");
	sql_sale_state.append(", SI.SALE_QTY ");
	sql_sale_state.append(",SI.SALE_AMT - (SI.SPECIAL_DC_AMT + SI.DC_AMT + SI.MBR_DC_AMT) AS SALE_AMOUNT ");
	sql_sale_state.append(",CPC.ITM_SHORT_NAME ");
	sql_sale_state.append(",FN_GET_STR_NAME(SH.STR_CODE,SH.CORP_CODE) AS STR_NAME ");
	sql_sale_state.append(",SUBSTR(SH.SALE_DT,0,4)||'-'||	SUBSTR(SH.SALE_DT,5,2)||'-'||	SUBSTR(SH.SALE_DT,7,2) AS SALE_DT ");
	sql_sale_state.append(",CPC.UNIT ");
	sql_sale_state.append("FROM    SL_SALE_HDR     SH ");
	sql_sale_state.append(",       SL_SALE_ITEM    SI ");
	sql_sale_state.append(",       CS_CUST         CC ");
	sql_sale_state.append(",       CD_PRODUCT_CMN  CPC ");
	sql_sale_state.append("WHERE   SH.CORP_CODE    =   SI.CORP_CODE ");
	sql_sale_state.append("AND     SH.SALE_DT      =   SI.SALE_DT ");
	sql_sale_state.append("AND     SH.STR_CODE     =   SI.STR_CODE ");
	sql_sale_state.append("AND     SH.POS_NO       =   SI.POS_NO ");
	sql_sale_state.append("AND     SH.TRXN_NO      =   SI.TRXN_NO ");
	sql_sale_state.append("AND     SH.CORP_CODE    =   CC.CORP_CODE(+) ");
	sql_sale_state.append("AND     SH.CUST_NO      =   CC.CUST_NO(+) ");
	sql_sale_state.append("AND     SI.CORP_CODE    = CPC.CORP_CODE ");
	sql_sale_state.append("AND     SI.ITM_CODE     = CPC.ITM_CODE ");
	sql_sale_state.append("AND     SI.SCAN_CODE    = CPC.SCAN_CODE ");
	sql_sale_state.append("AND     SH.CORP_CODE    = '^P_CORP_CODE^' ");
	sql_sale_state.append("AND     SH.STR_CODE     = '^P_STR_CODE^' ");
	sql_sale_state.append("AND     SH.SALE_DT      ='^P_SALE_DT^' ");
	sql_sale_state.append("AND     SH.TRXN_NO     ='^P_TRXN_NO^' ");
	sql_sale_state.append("AND     SH.POS_NO = '^P_POS_NO^' ");
	...
	sql_param.append(",	NVl(FN_GET_STR_NAME('^P_STR_CODE^','^P_CORP_CODE^'),'전체') AS STR_NAME ");
	sql_param.append(" ");
	sql_param.append(", ");
	sql_param.append("               SUBSTR('^P_SALE_DT^',0,4)||'-'||	SUBSTR('^P_SALE_DT^',5,2)||'-'||	SUBSTR('^P_SALE_DT^',7,2) AS SALE_DT ");
	sql_param.append(",	NVL('^P_POS_NO^','전체') AS POS ");
	sql_param.append(",	NVL('^P_TRXN_NO^','전체') AS TRXN_NO ");
...
class ReportHandler{
	...
	private String sale_state_SCAN_CODE;
	private long sale_state_SALE_PRC;
	private long sale_state_SALE_QTY;
	private long sale_state_SALE_AMOUNT;
	private String sale_state_ITM_SHORT_NAME;
	...
	private String param_TRXN_NO;

	//파라미터 변수 선언
	String P_CORP_CODE_Param;
	String P_STR_CODE_Param;
	String P_POS_NO_Param;
	String P_SALE_DT_Param;
	String P_TRXN_NO_Param;
	...
	private void getScriptFromv2(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			...
			//데이터 변수 할당
			sheet20_A2="점포명 : "+param_STR_NAME;
			sheet20_C2="매출일자 : "+param_SALE_DT;
			sheet20_E2="POS : "+param_POS;
			sheet20_G2="거래번호 : "+param_TRXN_NO;
			sheet20_K2 = param_STR_NAME;
			sheet20_L2 = param_TRXN_NO;
			sheet20_M2 = param_POS;
			sheet20_N2 = param_SALE_DT;
		}    //end if(bIsFirst)
	...
	private void getScriptFromfv1(boolean bIsPrint) throws Exception{
		...
		while(sale_state.next()){   //자동으로 생성되는 반복 밴드의 루프문
			...
			//데이터 변수 할당
			if(!sheet30_A2.equals(sale_state_SCAN_CODE)){
				if(!bIsFirst){
					sheet30_A2_Buf = sheet30_A2;
					sheet30_A2_Flag=true;
				}
				sheet30_A2=sale_state_SCAN_CODE;
			}
			sheet30_C2 = sale_state_ITM_SHORT_NAME;
			sheet30_D2 = sale_state_UNIT;
			sheet30_E2 = sale_state_SALE_PRC;
			sheet30_F2 = sale_state_SALE_QTY;
			sheet30_G2 = sale_state_SALE_AMOUNT;
			AI_BARCODE_OBJ7 = sale_state_SCAN_CODE;
	...
	private void writeScripth1( ) throws Exception{
		//스크립트문 생성
		objectWrite(DRAW_OBJECT, 523, 13, 44, 9, AI_STRING_OBJ2, "AI_STRING_OBJ2", "draw_style_1");
		objectWrite(DRAW_OBJECT, 450, 13, 42, 18, AI_STRING_OBJ3, "AI_STRING_OBJ3", "draw_style_1");
	}
	...
	private void writeScriptV1( ) throws Exception{
		...
		objectWrite(DRAW_OBJECT, nXCurPos+154, nYCurPos+37, 236, 0, "", "AI_HLINE_OBJ1", "draw_style_0");
	}
	...
	private void writeScriptfv1(  ) throws Exception{
		//스크립트문 생성
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet30_B2"), nYCurPos+mapTop.get("sheet30_B2"), mapWidth.get("sheet30_B2"), mapHeight.get("sheet30_B2"), "", "sheet30_B2", mapStyle.get("sheet30_B2"));
		...
		objectWrite(DRAW_OBJECT, nXCurPos+73, nYCurPos+3, 170, 22, AI_BARCODE_OBJ7, "AI_BARCODE_OBJ7", "draw_style_3");
	}
	...
	private void setFieldVariableFromsale_state() throws Exception{
		sale_state_POS_NO = GetB(sale_state.getString(1));
		sale_state_STR_CODE = GetB(sale_state.getString(2));
		sale_state_TRXN_NO = GetB(sale_state.getString(3));
		sale_state_SCAN_CODE = GetB(sale_state.getString(4));
		sale_state_SALE_PRC = sale_state.getLong(5);
		sale_state_SALE_QTY = sale_state.getLong(6);
		sale_state_SALE_AMOUNT = sale_state.getLong(7);
		sale_state_ITM_SHORT_NAME = GetB(sale_state.getString(8));
		sale_state_STR_NAME = GetB(sale_state.getString(9));
		sale_state_SALE_DT = GetB(sale_state.getString(10));
		sale_state_UNIT = GetB(sale_state.getString(11));
	}
	...
	private void setFieldInitFromsale_state(){
		...
		sale_state_STR_CODE = "";
		sale_state_TRXN_NO = "";
		sale_state_SCAN_CODE = "";
		sale_state_SALE_PRC = 0L;
		sale_state_SALE_QTY = 0L;
		sale_state_SALE_AMOUNT = 0L;
		sale_state_ITM_SHORT_NAME = "";
		sale_state_STR_NAME = "";
		sale_state_SALE_DT = "";
		sale_state_UNIT = "";
	}
	...
	private void setFieldVariableFromparam() throws Exception{
		...
		param_POS = GetB(param.getString(4));
		param_TRXN_NO = GetB(param.getString(5));
	}
	...
	private void setParam() {
		...
		P_POS_NO_Param=decoder.getParameterB("P_POS_NO");
		//P_POS_NO_Param=aiReplace(P_POS_NO_Param, str, "\n");
		mapParam.put("P_POS_NO", P_POS_NO_Param);
		...
		P_TRXN_NO_Param=decoder.getParameterB("P_TRXN_NO");
		//P_TRXN_NO_Param=aiReplace(P_TRXN_NO_Param, str, "\n");
		mapParam.put("P_TRXN_NO", P_TRXN_NO_Param);
	}
	...
	private void getScriptHeader(String paper, String size, String value, String orient, String margin) throws Exception{
		...
		manager.scriptWrite("AIReport", "scriptVersion|docVersion|style|title", "32|46|salesAnalReportTrPrint.style|회원매출상세내역현황", false);
	}
	...
	public void setCellInfoMap() throws Exception {
		...
		setCellInfoMap_Internal("sheet10_A1", 0, 0, 544, 39, "cell_style_0");
		setCellInfoMap_Internal("sheet20_A2", 0, 7, 67, 20, "cell_style_1");
		setCellInfoMap_Internal("sheet20_C2", 94, 7, 90, 20, "cell_style_1");
		setCellInfoMap_Internal("sheet20_E2", 240, 7, 67, 20, "cell_style_1");
		setCellInfoMap_Internal("sheet20_G2", 334, 7, 90, 20, "cell_style_1");
		setCellInfoMap_Internal("sheet30_A1", 0, 0, 70, 20, "cell_style_2");
		setCellInfoMap_Internal("sheet30_B1", 70, 0, 176, 20, "cell_style_2");
		setCellInfoMap_Internal("sheet30_C1", 246, 0, 107, 20, "cell_style_2");
		setCellInfoMap_Internal("sheet30_D1", 353, 0, 50, 20, "cell_style_2");
		setCellInfoMap_Internal("sheet30_E1", 403, 0, 50, 20, "cell_style_2");
		setCellInfoMap_Internal("sheet30_F1", 453, 0, 31, 20, "cell_style_2");
		setCellInfoMap_Internal("sheet30_G1", 484, 0, 59, 20, "cell_style_2");
		setCellInfoMap_Internal("sheet30_A2", 0, -1, 70, -1, "cell_style_3");
		setCellInfoMap_Internal("sheet30_B2", 70, 0, 176, 46, "cell_style_4");
		setCellInfoMap_Internal("sheet30_C2", 246, 0, 107, 46, "cell_style_5");
		setCellInfoMap_Internal("sheet30_D2", 353, 0, 50, 46, "cell_style_3");
		setCellInfoMap_Internal("sheet30_E2", 403, 0, 50, 46, "cell_style_6");
		setCellInfoMap_Internal("sheet30_F2", 453, 0, 31, 46, "cell_style_7");
		setCellInfoMap_Internal("sheet30_G2", 484, 0, 59, 46, "cell_style_8");
		setCellInfoMap_Internal("sheet30_A3", 0, 0, 484, 20, "cell_style_9");
		setCellInfoMap_Internal("sheet30_G3", 484, 0, 59, 20, "cell_style_10");
	}
}
%>
