...
public void jspInit() {
	...
	sql_ven_day_state.append("SELECT C.PUR_CFM_DT ");
	sql_ven_day_state.append(",			C.STR_CODE ");
	sql_ven_day_state.append(",			C.STR_NAME ");
	sql_ven_day_state.append(",			NVL(SUM(C.PUR_WPRC_A),0) AS PUR_WPRC_A ");
	sql_ven_day_state.append(",			NVL(SUM(C.PUR_WPRC_B),0) AS PUR_WPRC_B ");
	sql_ven_day_state.append(",			NVL(SUM(C.PUR_PAY_AMT),0) AS PUR_PAY_AMT ");
	sql_ven_day_state.append("FROM ( ");
	sql_ven_day_state.append("SELECT A.PUR_CFM_DT, ");
	sql_ven_day_state.append("			A.STR_CODE , ");
	sql_ven_day_state.append("			A.STR_NAME, ");
	sql_ven_day_state.append("			ROUND(NVL(SUM(PUR_WPRC_1),0),0) AS PUR_WPRC_A, ");
	sql_ven_day_state.append("			ROUND(NVL(SUM(PUR_WPRC_2),0),0) AS PUR_WPRC_B , ");
	sql_ven_day_state.append("			ROUND(NVL(SUM(A.PUR_WPRC_3),0),0) AS PUR_PAY_AMT ");
	sql_ven_day_state.append("FROM ( ");
	sql_ven_day_state.append(" ");
	sql_ven_day_state.append("				SELECT     PH.PUR_CFM_DT ");
	sql_ven_day_state.append("				,PD.STR_CODE ");
	sql_ven_day_state.append("				,CS.STR_NAME ");
	sql_ven_day_state.append("				, CASE WHEN PH.PUR_GB = '1' THEN ");
	sql_ven_day_state.append("						NVL(PD.DEC_QTY * (PD.PUR_WPRC),0) ");
	sql_ven_day_state.append("				END AS PUR_WPRC_1 ");
	sql_ven_day_state.append("				, CASE WHEN PH.PUR_GB = '2' THEN ");
	sql_ven_day_state.append("						NVL(PD.DEC_QTY * (PD.PUR_WPRC),0) * -1 ");
	sql_ven_day_state.append("				END AS PUR_WPRC_2 ");
	sql_ven_day_state.append("				,'0' AS PUR_WPRC_3 ");
	sql_ven_day_state.append("                    FROM   PO_PURCH_HDR PH ");
	sql_ven_day_state.append("                    ,      PO_PURCH_DTL PD ");
	sql_ven_day_state.append("                    ,			CD_STORE CS ");
	sql_ven_day_state.append("                    WHERE ");
	sql_ven_day_state.append("                    		  PH.CORP_CODE = '^P_CORP_CODE^' ");
	sql_ven_day_state.append("                    AND    PH.PUR_CFM_DT IS NOT NULL ");
	sql_ven_day_state.append("                    AND	  PH.PUR_CFM_DT BETWEEN  '^P_SALE_DT^' ||'01' AND  '^P_SALE_DT^'||'31' ");
	sql_ven_day_state.append("                    AND	  PH.STR_CODE = CS.STR_CODE ");
	sql_ven_day_state.append("                    AND	  PH.CORP_CODE = CS.CORP_CODE ");
	sql_ven_day_state.append("                    AND    PD.CORP_CODE = PH.CORP_CODE ");
	sql_ven_day_state.append("                    AND	  PD.STR_CODE = PH.STR_CODE ");
	sql_ven_day_state.append("                    AND    PD.PUR_DT = PH.PUR_DT ");
	sql_ven_day_state.append("                    AND    PD.STR_CODE = NVL('^P_STR_CODE^',PD.STR_CODE) ");
	sql_ven_day_state.append("                    AND    PD.SLIP_NO = PH.SLIP_NO ");
	sql_ven_day_state.append("        ) A ");
	sql_ven_day_state.append("                    group by A.PUR_CFM_DT, A.STR_CODE, A.STR_NAME ");
	sql_ven_day_state.append(" ");
	sql_ven_day_state.append(" ");
	sql_ven_day_state.append(" ");
	sql_ven_day_state.append("          UNION ");
	sql_ven_day_state.append(" ");
	...
	sql_ven_day_state.append(",			ROUND(SUM(B.PUR_WPRC_1),0) AS PUR_WPRC_A ");
	sql_ven_day_state.append(",			ROUND(SUM(B.PUR_WPRC_2),0) AS PUR_WPRC_B ");
	...
	sql_ven_day_state.append("		       AND	  SDS.SALE_DT BETWEEN  '^P_SALE_DT^' ||'01' AND  '^P_SALE_DT^'||'31' ");
	...
	sql_ven_day_state.append("		ORDER BY PUR_CFM_DT, STR_CODE ");
	sql_ven_day_state.append("		)C ");
	sql_ven_day_state.append("		GROUP BY C.PUR_CFM_DT, C.STR_CODE, C.STR_NAME ");
...
class ReportHandler{
	...
	private void getScriptFromsh3_fv1(boolean bIsPrint) throws Exception{
		...
		while(ven_day_state.next()){   //자동으로 생성되는 반복 밴드의 루프문
			...
			//데이터 변수 할당
			sheet30_A3 = ven_day_state_PUR_CFM_DT;
	...
	private void getScriptHeader(String paper, String size, String value, String orient, String margin) throws Exception{
		...
		manager.scriptWrite("AIReport", "scriptVersion|docVersion|style|title", "32|85|영업정보_매입관리_협력업체_전체_점별출력.style|회원매출상세내역현황", false);
	}
	...
	public void setCellInfoMap() throws Exception {
		...
		setCellInfoMap_Internal("sheet30_A3", 0, 0, 82, 20, "cell_style_5");
		setCellInfoMap_Internal("sheet30_B3", 82, -1, 84, -1, "cell_style_6");
		setCellInfoMap_Internal("sheet30_C3", 166, -1, 87, -1, "cell_style_7");
