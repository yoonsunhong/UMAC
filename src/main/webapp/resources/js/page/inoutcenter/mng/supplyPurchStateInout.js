...
'<rMateGrid>\
	...
	<SpanCellAttribute id="sum1CellAttr" colNo="0" styleName="subTotalStyle" backgroundColor="#dddddd" colSpan="5" />\
	...
	<DataGrid id="dg1" horizontalScrollPolicy="auto" sortableColumns="true">\
		<columns>\
			<DataGridColumn dataField="STR_NAME" 			id="STR_NAME"		headerText="' + storNm + '" 			textAlign="center" width="70" />\
			<DataGridColumn dataField="VEN_CODE"  			id="VEN_CODE"		headerText="' + venCode + '"  		textAlign="center" width="130"	/>\
			<DataGridColumn dataField="VEN_NAME"  			id="VEN_NAME"		headerText="' + venName + '"  		textAlign="center" width="130"	/>\
			<DataGridColumn dataField="GRE_GB_NM"  		id="GRE_GB_NM"		headerText="거래구분"  					textAlign="center" width="130"	/>\
			<DataGridColumn dataField="PUR_GB_NM"  		id="PUR_GB_NM"		headerText="전표구분"  					textAlign="center" width="130"	/>\
			<DataGridColumn dataField="PUR_WPRC2"  		id="PUR_WPRC2"		headerText="면세원가"  					textAlign="right" width="165" formatter="{numfmt}"/>\
			<DataGridColumn dataField="PUR_WPRC1"  		id="PUR_WPRC1"		headerText="과세원가"  					textAlign="right" width="165" formatter="{numfmt}"/>\
			<DataGridColumn dataField="PUR_WAMT"  		id="PUR_WAMT"		headerText="원가합계"  					textAlign="right" width="165" formatter="{numfmt}"/>\
			<DataGridColumn dataField="PUR_WVAT1"  		id="PUR_WVAT1"		headerText="부가세"  					textAlign="right" width="165" formatter="{numfmt}"/>\
			<DataGridColumn dataField="PUR_WAMT2"  		id="PUR_WAMT2"		headerText="구입금액합"  				textAlign="right" width="165" formatter="{numfmt}"/>\
			<DataGridColumn dataField="BOT_SUM"  			id="BOT_SUM"			headerText="공병보증금"  				textAlign="right" width="165" formatter="{numfmt}"/>\
		</columns>\
		<dataProvider>\
			...
			<mergingFields>\
		        <SpanMergingField name="VEN_CODE">\
		            <SpanSummaryRow label="소계"   labelDataField="STR_NAME" rowAttribute="{sumRowAttr_V}" cellAttribute="{sum1CellAttr}">\
		                ...
		                <SpanSummaryField dataField="PUR_WPRC1"		summaryOperation="SUM" />\
		                <SpanSummaryField dataField="PUR_WAMT"		summaryOperation="SUM" />\
		                <SpanSummaryField dataField="PUR_WVAT1"		summaryOperation="SUM" />\
		...
		<footers>\
			<DataGridFooter  height="26" color="#000" backgroundColor="#c5c5c5" borderTopColor="#858585" borderTopWidth="1" borderTopStyle="solid" fontWeight="normal" rowAttribute="{sumRowAttr_T}" cellAttribute="{sumTotalCellAttr}">\
				<DataGridFooterColumn label="합계"  textAlign="center"/>\
				<DataGridFooterColumn/>\
				<DataGridFooterColumn/>\
				<DataGridFooterColumn/>\
				<DataGridFooterColumn/>\
				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{PUR_WPRC2}" formatter="{numfmt}" textAlign="right" />\
				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{PUR_WPRC1}" formatter="{numfmt}" textAlign="right" />\
				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{PUR_WAMT}" formatter="{numfmt}" textAlign="right" />\
				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{PUR_WVAT1}" formatter="{numfmt}" textAlign="right" />\
				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{PUR_WAMT2}" formatter="{numfmt}" textAlign="right" />\
				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{BOT_SUM}" formatter="{numfmt}" textAlign="right" />\
			</DataGridFooter>\
	    </footers>\
	</DataGrid>\
</rMateGrid>';
...
//	showLoadingBar1();
	...
	jQuery.ajax({ 
	    url:"/supplyPurchStateInout_list.do",         
	    type:"POST",
		datatype:"json",
		data: loadData,
		beforeSend : function(){ 	    	
...
function fnExcelExport(){
	...
	*/
	if($('#P_STR_CODE').val() == "" ){
		...
		dataGrid1.exportFileName = "매입집계표(대출입포함)_"+nowDate+".xlsx";
...
function btn_print(){
	...
	;
	 // AIViewer 파라미터
	window.open("aireportSupplyPurchStateInout.do"+params,'AIViewer','width=1180,height=900,top=10,left=10,toolbar=no,menubar=no,lacation=no,scrollbars=no,status=no,resizable=yes');
}
