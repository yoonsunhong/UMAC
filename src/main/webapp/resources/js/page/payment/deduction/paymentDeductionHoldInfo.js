...
function gridReadyHandler(id) {
	if (id == "grid1") {
		...
		var itemClickHandler = function(event) {
			...
			$('#P_HOLD_DT' ).val(dataRow1.HOLD_DT);
			$('#P_HOLD_AMT' ).val(dataRow1.HOLD_AMT);
			...
			$('#PAY_CFM_YN' ).val(dataRow1.PAY_CFM_YN);
...
'<rMateGrid>\
	...
	<DataGrid id="dg1" sortableColumns="true">\
		<columns>\
			...
			<DataGridColumn dataField="HOLD_YM"  		headerText="' + holdYm + '"  		textAlign="center" 	width="100"	/>\
			<DataGridColumn dataField="VEN_CODE"  		headerText="' + venCode + '"  		textAlign="center" 	width="100"	/>\
			<DataGridColumn dataField="VEN_NAME"  		headerText="' + supply + '"			textAlign="left" 	width="130"	/>\
			...
			<DataGridColumn dataField="PAY_CLOSE"		headerText="지불마감" 		visible="false"	/>\
		</columns>\
		<dataProvider>\
...
function fn_comm_supply_callback(dataRow){
	...
	//협력업체의 지불주기/지불차수 선택
	getPayNum(dataRow.VEN_CODE, 'S_PAY_CON','S_PAY_SEQ');
}
...
function fnUpdate(){
	...
	//최초 로딩시 그리드1 데이터 조회 및 그리드 데이터 셋팅
	jQuery.ajax({ 
	    url:"/paymentDeductionHoldUpdate.do",         
	    type:"POST",
		datatype:"json",
		data: loadData,
		beforeSend : function(){ 	    	
...
function chgPay( PAY_GB ) { 
	if( PAY_GB == "PAY_CON")
	{
		...
		{    
			...
			$("#S_PAY_CON").empty();
			$("#S_PAY_CON option").remove();
			$("#S_PAY_CON" ).append('<option value="">' + all + '</option>');
			getCommonCodeSelectBoxList("S_PAY_CON",   "PAY_CON"); // 지불주기  조회조건
...
function init() {
	$('#P_HOLD_AMT').number( true );
	getCommonCodeSelectBoxList("P_HOLD_CODE",   "HOLD_CODE");    //	공제코드
	getCommonCodeSelectBoxList("S_PAY_CON",   "PAY_CON");
	...
	$("input[name=I_VEN_NAME]").keydown(function (key) { 
    	...
    	if(venName == null || venName == ''){
    		...
    		getCommonCodeSelectBoxList("P_PAY_CON",   "PAY_CON"); // 지불/주기  조회조건
	    ...
	    success : function(data) {
			...
			for(var i = 0; i < list.length; i++){
				if(type == 'A'){
					...
					$("#P_PAY_CON").append('<option value="'+ list[i].STR_CODE +'">'+ list[i].STR_CODE +': '+ list[i].STR_NAME +'</option>');
					if(i == 0){
						fnSelectPaymentBoxList('C',list[0].STR_CODE);
					}
				}else{
					$("#P_PAY_SEQ").append('<option value="'+ list[i].STR_CODE +'">'+ list[i].STR_CODE +': '+ list[i].STR_NAME +'</option>');
				}
...
function fnValidation(){
	...
	if($.trim($("#P_HOLD_CODE").val() ) == null || $.trim($("#P_HOLD_CODE").val() ) == "")
	{
		alert(holdCode + msgConfirm);
		$("#P_HOLD_CODE").focus();
		return false;
	}
	
	if($.trim($("#P_HOLD_DT").val() ) == null || $.trim($("#P_HOLD_DT").val() ) == "")
	{
	...
	if($.trim($("#P_HOLD_AMT").val() ) == null || $.trim($("#P_HOLD_AMT").val() ) == "" || $.trim($("#P_HOLD_AMT").val() ) == "0")
	{
		alert(holdAmt + msgConfirm);
		$("#P_HOLD_AMT").focus();
		return false;
	}
