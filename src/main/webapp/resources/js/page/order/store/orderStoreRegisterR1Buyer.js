...
//	getCommonCodeSelectBoxList("ITM_GB"    ,   "ITM_GB"		);   
	...
	// 배송구분에서 R1 만 남긴다
	$("#ROUTE_GB  option[value!='R1']").remove(); 
	
	getCommonCodeSelectBoxList("S_ROUTE_GB"  ,   "ROUTE_GB"	); 
	// 조회조건의 배송구분에서  R1 만 남긴다
	$("#S_ROUTE_GB  option[value!='R1']").remove(); 
...
//	if( $("#SESSION_STR_CODE").val()    == "")  // 관리자 로그인시 다보여줌
//	{ 
...
'<rMateGrid>\
	...
	<DataGrid id="dg1"  sortableColumns="false"  editable="true" horizontalScrollPolicy="auto" showDeletedRows="true"     doubleClickEnabled="false" selectionMode="multipleRows" >\
		<columns>\
			...
			<DataGridColumn  id="SLIP_NO"		 dataField="SLIP_NO"    	headerText="발주번호"		editable="false"		textAlign="center"	 width="120"    		/>\
...
//	var PUR_DT = new CommDateManager().after(0, 0, 2).getDate("yyyy-mm-dd"); // 일주일전/후  before / after(년,월,일) 
//	$("#PUR_DT").val( PUR_DT );
...
//    xmlDoc.getElementsByTagName("GRIDROW")[0].appendChild(	   ITM_GB	     );           
	...
	jQuery.ajax({ 
	    url:"/orderStoreProductRegisterR1Buyer.do",          
	    type:"POST",
		datatype:"xml",
		async:true,
...
function orderDel(){
    ...
    //  발주 삭제   
	jQuery.ajax({ 
	    url:"/orderDelR1Buyer.do",          // orderStoreProductRegister 
	    type:"POST",
		datatype:"xml",
		async:false,
...
function btn_clear(){
	...
	// 배송구분 초기화 후, 선택 넣고 다시 그려주고, R1 만 넣는다.
...
function btn_search(){
	...
	jQuery.ajax({ 
	    url:"/orderHeadSearchR1Buyer.do",         //  productStoreSearchList   productBoxSearchList 
	    type:"POST",
		datatype:"json",
		async:false,
...
{ 
	...
	jQuery.ajax({ 
	    url:"/getCommonMgmtEntryR1Buyer.do",           
	    type:"POST",
		datatype:"json",
		async:false,
	 	data: {	"CD_CL"  : "ROUTE_GB"
...
function orderHeadDtailInfo(  STR_CODE , SLIP_NO   )
{
	...
	// 발주헤더 정보 보여주기
	jQuery.ajax({ 
	    url:"/orderHeadInfoR1Buyer.do",         // productCustomerInfoSelect
	    type:"POST",
		datatype:"json",
		async:false,
...
//	 		$('#ITM_GB').val(	data[0].ITM_GB		); 
	...
	jQuery.ajax({ 
		...
		data: {   "STR_CODE"  : $("#S_STR_CODE").val()  , "SLIP_NO"  : SLIP_NO    },
		success:function(data){  
			// gridApp2.setData( data );  
	...
	if(   dataField == "SCAN_CODE"  ) {  
		...
		if( typeof SCAN_CODE   != 'undefined' )
		{
			...
			{ 	
				...
				jQuery.ajax({ 
				    url:"/orderStoreProductSelectR1Buyer.do",              
				    type:"POST",
					datatype:"json",
					async:false,
