	...
	'<rMateGrid>\
		...
		<NumberFormatter id="numfmt" useThousandsSeparator="true"/>\
			<DataGrid id="dg1" sortableColumns="true" horizontalScrollPolicy="auto"    selectionMode="multipleCells"       >\
				<columns>\
					<DataGridColumn id="CD"   			dataField="CD" 					headerText="'+codeTop+'" 			labelJsFunction="labelFunc"  textAlign="center"		width="80"/>\
					...
					<DataGridColumn id="PUR_WPRC"   	dataField="PUR_WPRC" 			headerText="매입원가" 					textAlign="right"		formatter="{numfmt}"/>\
					<DataGridColumn id="RTN_WPRC"   	dataField="RTN_WPRC" 			headerText="매입반품" 				visible="false" 		textAlign="right"		formatter="{numfmt}"/>\
					<DataGridColumn id="SALE_SPRC"   	dataField="SALE_SPRC" 			headerText="매출금액" 					textAlign="right"		formatter="{numfmt}"/>\
					<DataGridColumn id="INV_ADJ_SPRC"   dataField="INV_ADJ_SPRC" 		headerText="조정금액" 			visible="false" 		textAlign="right"		formatter="{numfmt}"/>\
					<DataGridColumn id="DOUT_SPRC"   	dataField="DOUT_SPRC" 			headerText="대출금액" 			visible="false" 		textAlign="right"		formatter="{numfmt}"/>\
					<DataGridColumn id="DIN_SPRC"   	dataField="DIN_SPRC" 			headerText="대입금액" 			visible="false" 		textAlign="right"		formatter="{numfmt}"/>\
...
function labelFunc_SALE_WPRC_PER(item, value, column)
{
	...
	if(pTab =="2"){
		...
		for (var i = 0; i < src.length; i++){
			...
			{
				gridRoot2.setItemFieldAt(   ( 100 -  gridRoot2.getItemFieldAt( i , "PROFIT")  ).toFixed(2)   , i, "SALE_WPRC_PER");
		...
		for (var i = 0; i < src.length; i++){
			...
			if(dataSet.LRG_CODE == "소계"){
				...
				gridRoot2.setItemFieldAt( sum , i, "SALE_WPRC_PER");
				gridRoot2.setItemFieldAt((100 - sum).toFixed(2) , i, "PROFIT");
			} 
		}
	...
	if(pTab =="3"){
		...
		for (var i = 0; i < src.length; i++){
			...
			if(  dataSet.LRG_CODE == "01"  ||  dataSet.LRG_CODE == "02" ||  dataSet.LRG_CODE == "03"   )
			{
				gridRoot3.setItemFieldAt(   ( 100 -  gridRoot3.getItemFieldAt( i , "PROFIT")  ).toFixed(2)   , i, "SALE_WPRC_PER");
		...
		for (var i = 0; i < src.length; i++){
			...
			if(dataSet.LRG_CODE == "소계"    || dataSet.MID_CODE == "소계"   ){
				...
				gridRoot3.setItemFieldAt( sum , i, "SALE_WPRC_PER");
				gridRoot3.setItemFieldAt((100 - sum).toFixed(2) , i, "PROFIT");
			}
		}
	}
