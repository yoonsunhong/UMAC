...
//	var SALE_STR_DT =new CommDateManager().before(0,1, 0).getDate("yyyy-mm-01"); // 일주일전 before(년,월,일)
...
function itemDataChangeHandler2(event) {
	...
	if(dataField == "DPOT_FLAG"){
		...
		if(newValue == "01"){
			...
			var PREPAY_ZAN_AMT = gridRoot5.getItemFieldAt(0, "PREPAY_ZAN_AMT");
			gridRoot2.setItemFieldAt( PREPAY_ZAN_AMT , rowIndex, "DPOT_AMT");
		}else if(newValue == "06"){
			var PREPAY_ZAN_AMT = gridRoot5.getItemFieldAt(0, "PREPAY_ZAN_AMT");
			gridRoot2.setItemFieldAt( PREPAY_ZAN_AMT , rowIndex, "DPOT_AMT");
		}else if(newValue == "05"){
...
function dataCompleteHandler2(event) {
	...
	//선수금정산(현금),선수금정산(카드)  넣어주기위해 하드코딩으로 공통코드로 삽입 X 없기에
	var defaultValue = {'label':select, 'code':''};
	dataProvider.push(defaultValue);
	dataProvider.push({'label':"현금입금", 'code':'01'});
	dataProvider.push({'label':"신용카드", 'code':'02'});
	dataProvider.push({'label':"포인트입금", 'code':'03'});
	dataProvider.push({'label':"선수금정산(현금)", 'code':'04'});
	dataProvider.push({'label':"선수금정산(카드)", 'code':'06'});
	dataProvider.push({'label':"계좌이체", 'code':'05'});
...
function itemFunc(rowIndex, columnIndex, item, dataField){
	...
	if(gridRoot2.getItemFieldAt(rowIndex, "DPOT_FLAG") == "04" && columnIndex == 6  ){
		return false;
	}
	...
	'<rMateGrid>\
		...
		<DataGrid id="dg2" editable="true" horizontalScrollPolicy="auto" showDeletedRows="true" itemEditBeginningJsFunction="itemFunc" editalbe="true" showDataTips="true">\
			<groupedColumns>\
				...
				<DataGridColumn id="DPOT_FLAG"			dataField="DPOT_FLAG"  			headerText="' + dpotFlag +'" textAlign="center" itemEditor="ComboBoxEditor" editorDataField="selectedDataField" itemRendererDataField="code" itemRenderer="DataProviderItem" width="100" />\
...
//			alert("totalDpotAmt:"+totalDpotAmt);
	...
	//totalDpotAmt = 입금 미완료 합계
	//totalPayAmt = 입금 미완료 N 외상매출 - 입금액 = N인것들의 미수금의 합
	if(prepayZanAmt < 0){
		prepayZanAmt = 0;
	}
//	
//	alert("정산진행시 남는 잔액 : " + prepayZanAmt);
//	alert("입급금액 : " + Number(totalDpotAmt));
//	alert("매출금액 : " + Number(totalPayAmt));
//	
	if(Number(totalPayAmt) == 0 && (dpotFlag=="04" || dpotFlag=="06" )){
		alert("매출금액이 0원일땐, 선수금정산을 할 수 없습니다.");
		return;
	}
...
//			alert("zanAmt"+zanAmt);
//			alert("dataRow.PAY_AMT"+Number(dataRow.PAY_AMT));
//			alert("dataRow.DPOT_AMT"+Number(dataRow.DPOT_AMT));
//			alert("payAmt"+payAmt);
			
			// 알림톡 발송 대상 수정전 key 값 담기
			if(gridRoot2.getItemFieldAt(x, "DPOT_FLAG") != "04" && gridRoot2.getItemFieldAt(x, "DPOT_FLAG") != "06"){
				var alimtalData = {
...
//				alert("zanAmt >= payAmt zanAmt : "+zanAmt);
...
function btnSaveOverCharge(){
	...
	*/
	...
	{   
		if(gridRoot2.getItemFieldAt(i, "INPUT_YN") != "N" ){
			alert(getXmlString(   gridRoot2.getItemAt(i)   ));
			gridXmlData2 = gridXmlData2 + getXmlString(   gridRoot2.getItemAt(i)   );
		}
	}
