...
'<rMateGrid>\
	...
	<DataGrid id="dg1"  sortableColumns="true"  editable="true" horizontalScrollPolicy="auto" showDeletedRows="false"    selectionMode="multipleCells"   doubleClickEnabled="false" >\
		<columns>\
			...
			<DataGridColumn  id="SCM_QTY"	 	 dataField="SCM_QTY" 		headerText="SCM납품수량"  	editable="true"		    visible="false"  textAlign="right"     maxChars="6" type="int"   width="95"    formatter="{numfmt}"  showEditableIcon="always" itemRenderer="EditableIconItem" 	/>\
			<DataGridColumn  id="PUR_QTY"	 	 dataField="PUR_QTY" 		headerText="매입수량"  	editable="true"		    visible="false"  textAlign="right"     maxChars="6" type="int"   width="70"    formatter="{numfmt}"  showEditableIcon="always" itemRenderer="EditableIconItem" 	/>\
			...
			<DataGridColumn  id="PUR_WAMT"       dataField="PUR_WAMT"       headerText="매입단가"		editable="true"			showEditableIcon="always"   itemRenderer="EditableIconItem" textAlign="right"	 width="98"    formatter="{numfmt}"		/>\
			<DataGridColumn  id="PUR_WPRC"	 	 dataField="PUR_WPRC"       headerText="공급가"  		visible="true" editable="true"		showEditableIcon="always"   itemRenderer="EditableIconItem" textAlign="right"	 width="85"   formatter="{numfmt}"  	/>\
			<DataGridColumn  id="PUR_WVAT"		 dataField="PUR_WVAT"       headerText="부가세" 		visible="true" editable="true"		showEditableIcon="always"   itemRenderer="EditableIconItem" textAlign="right"	 width="85"    formatter="{numfmt}"		/>\
	<DataGridColumn  id="PUR_WPRC_TOT"	 	 dataField="PUR_WPRC_TOT"       headerText="공급가(*수량)"  	editable="true"		showEditableIcon="always"   itemRenderer="EditableIconItem" textAlign="right"	 width="105"   formatter="{numfmt}"  	/>\
	<DataGridColumn  id="PUR_WVAT_TOT"		 dataField="PUR_WVAT_TOT"       headerText="부가세(*수량)" 	editable="true"		showEditableIcon="always"   itemRenderer="EditableIconItem" textAlign="right"	 width="105"    formatter="{numfmt}"		/>\
	<DataGridColumn  id="PUR_WAMT_TOT"		 dataField="PUR_WAMT_TOT"       headerText="합계금액" 		editable="true"	    	showEditableIcon="always"   itemRenderer="EditableIconItem" textAlign="right"	 width="105"    formatter="{numfmt}"		/>\
...
var layoutStr2 =
	...
	<DateFormatter id="datefmt" formatString="YYYY-MM-DD"/>\
		<NumberFormatter   	 id="numfmt" 	useThousandsSeparator="true"   />\
		<DataGrid id="dg1" sortableColumns="true"  dragSelectable="true"    editable="true" horizontalScrollPolicy="auto"  selectionMode="multipleRows" >\
			<columns>\
				<DataGridSelectorColumn id="selector2" width="40" textAlign="center"  headerText=""  allowMultipleSelection="true"  backgroundColor="#EDEDF0" allowAllSelection="true" secondLabelJsFunction="secondLabelFunc" />\
	...
	<DataGridColumn  id="UEMP_NO"	  dataField="UEMP_NO"    	headerText="최종수정자"  	visible="true"  		editable="false"		textAlign="center"   width="112"      	/>\
				...
				<DataGridColumn  id="PAY_CLOSE"	 	  dataField="PAY_CLOSE"    		headerText="지불마감"  	visible="false"  		editable="false"		formatter="{datefmt}" textAlign="center"   width="80"      	/>\
...
function secondLabelFunc(item, value, column) {
	 ...
	 if (   item.PUR_CFM_DT == undefined   ||   item.CFM_DT != undefined )
		 return false;
	 else
		 return true;
	};
...
//	$("#ROUTE_GB  option[value!='"+$("#ROUTE_GB").val()+"']").remove(); 
	...
	{
		 DEC_QTY_COL.editable  		   = true; // 수량 활성화
		 PUR_WAMT_COL.editable 		   = true;  // 금액 활성화 
		 
		 PUR_WPRC_COL.editable         =  true;   // 공급가
		 PUR_WVAT_COL.editable         =  true;   // 부가세 
		 PUR_WAMT_TOT_COL.editable     =  true;   // 합계금액  
...
//	[ 매입 마감처리  ] 
//	1. 확정 -> 매입마감
//	2. PUR_CFM_DT 가 not null이고 , 지불마감이 안된 전표( PY_PAY_CLOSE )에 대해서만 마감처리함( 마감처리 : CFM_DT에 오늘날짜 업데이트 ).  마감후 재 업데이트 금지
//	3. 이때, 상품 상세는 수량은 수정 불가이고,  금액(원가, 부가세) 만 수정가능  
//	4. 금액 수정시 PO_PURCH_ADJUST 에 insert ( 한개의 ROW 에 이전 금액과 새 금액을 넣는다.)
//	   ( 마감전는 여러번 금액을 수정 가능하고, 이때는 수정된 금액만 업데이트 해준다. <- search 해서 insert 된게 있으면 업데이트이다.)
