...
function getGridData() {
	...
	if($.trim($("#S_ITM_NAME").val() ) == null || $.trim($("#S_ITM_NAME").val() ) == "")
		$("#S_ITM_CODE").val("");
	$("#S_ITM_NAME_LIKESEARCH").val($.trim($("#S_ITM_NAME_LIKESEARCH").val()));
...
function btn_comm_store_search(){
	if($('#S_STR_CODE').val()==""){
		$('#P_STR_CODE').val(P_STR_CODE_HISTORY);
	}else{
		$("#pop_wrap input[name='P_STR_CODE']").val($("#S_STR_CODE").val());
		$("#pop_wrap input[name='STORE_NAME']").val($("#S_STR_CODE option:selected").text());
	}
	
	//console.log("P_STR_CODE : " + $('#P_STR_CODE').val());
	//console.log("S_STR_CODE : " + $('#S_STR_CODE').val());
	//console.log("P_STR_CODE_HISTORY : " + P_STR_CODE_HISTORY);
...
function init() {
	// 점포코드 콤보 가져오기
	$("#search_frm select[id='S_STR_CODE']").append('<option value="">'+ all +'</option>');
	getStoreCode("S_STR_CODE");
	
	P_STR_CODE_HISTORY = $('#P_STR_CODE').val();
